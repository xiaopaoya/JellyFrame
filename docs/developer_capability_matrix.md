...
| `overflow` | Subset | `visible`, `hidden`, `clip`, `auto`, `scroll`; clipping creates layer boundaries. V0 native scroll containers support fixed-size vertical `auto`/`scroll` regions with host-provided scroll offsets, clipped paint and hit testing. The Win32 shell maps wheel/arrow/drag default actions to the nearest scrollable container before falling back to page scroll, repaints only the container viewport when the dirty area stays within budget, and applies a bounded host-side inertia tail while frame presentation is allowed. Visible scrollbars, horizontal scroll and strip-level scroll blit are deferred. |
