...
| Form properties / `FormData` | Subset | Common IDL properties on relevant controls: `value`, `defaultValue`, input `defaultChecked`, `type`, `name`, `placeholder`, `required`, `checked`, `selectedIndex`, `readOnly`, `maxLength`, `minLength`, `min`, `max`, `step`, `willValidate`, `validationMessage`, `validity`, textarea `rows`/`cols`/`wrap`/`textLength`, select `size`, option `label`/`defaultSelected`/`value`/`text`/`index`, optgroup `label`, and progress/meter numeric `value`/`min`/`max` plus meter `low`/`high`/`optimum` and progress `position`. Controls provide `checkValidity()`, `reportValidity()` and `setCustomValidity(message)`; form wrappers additionally provide `requestSubmit()` and cancellable `reset()`. `new FormData(form)` and `append`/`set`/`delete`/`get`/`getAll`/`has`/`forEach` cover string form entries. `forEach` passes `(value, name, formData)` in entry order and uses a bounded entry snapshot, so entries added by its callback wait for the next call. The `ValidityState` snapshot intentionally contains only the implemented flags; labels collections, selection APIs, picker UI, file entries, iterator methods and browser form navigation are deferred. |
...
| Package background placement | Subset | One package-local `background-image: url("/assets/image.bmp")` can use `background-size: cover`/`contain`/`100% 100%`, the existing simple `background-position` subset and `background-repeat: no-repeat`. The selected fit, position and sampling mode are packed into the existing background-image word and carried by the existing image display command, so this adds no `Style` fields, decoder, cache or work to pages without a background image. Tiling, multiple image layers and arbitrary size expressions remain deferred. |
