...
| XHR/fetch/storage | Deferred | Host services exist only as C++ queues/mocks today. Planned user-facing syntax must be a standard subset: async `XMLHttpRequest` before `fetch()`, and tiny `localStorage` only on profiles that can keep calls non-blocking. |
