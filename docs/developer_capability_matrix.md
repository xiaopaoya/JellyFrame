...
| XHR/fetch/storage | Partial | Scripting builds support async `XMLHttpRequest` GET V0. `fetch()` waits for bounded Promise/microtask support, and tiny `localStorage` should be exposed only on profiles that can keep calls non-blocking. |
