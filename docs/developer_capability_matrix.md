...
| `HTMLElement.click()` | Subset | Dispatches a synthetic mouse-like `click` with zero coordinates. For JellyFrame controls it also runs the existing bounded activation path: checkbox/radio/select state changes emit `input`/`change`, and uncanceled `summary.click()` toggles parent `details`. It does not implement browser navigation/submission activation. |
...
| Form properties | Subset | Common IDL properties on relevant controls: `value`, `defaultValue`, input `defaultChecked`, `type`, `name`, `placeholder`, `required`, `checked`, `selectedIndex`, `readOnly`, `maxLength`, `minLength`, `min`, `max`, `step`, textarea `rows`/`cols`/`wrap`/`textLength`, select `size`, option `label`/`defaultSelected`/`value`/`text`/`index`, optgroup `label`, and progress/meter numeric `value`/`min`/`max` plus meter `low`/`high`/`optimum` and progress `position`. Full validity state, labels collections, selection APIs, picker UI and browser form submission are deferred. |
