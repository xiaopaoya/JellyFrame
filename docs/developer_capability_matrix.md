...
| Device Runtime / JFDP | Reference implementation | `DeviceInstallTransaction` and `JFDP/1` framing provide a bounded, platform-neutral install transaction and message integrity checks. CLI `device --transport reference` operates only on the desktop registry reference endpoint and does not claim that a physical board is connected. Discovery, flash/storage, display/touch, logs and telemetry remain official-port responsibilities. |
