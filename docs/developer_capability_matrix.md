...
| Device Runtime / JFDP | Contracts plus one wire-accepted port | `DeviceInstallTransaction`, `DeviceCapabilitySnapshot`, versioned request/result payload codecs and `JFDP/1` framing provide bounded install/lifecycle control, capability data, stable status/progress results and message integrity checks. WS147 native USB Serial/JTAG passed the JFDP/1 wire matrix for one documented image/configuration. It is not a Developer Image lifecycle release: persistent registry/staging, launcher recovery, typed library/log records and author-tool integration remain Device OS work under `device_image_lifecycle_port_acceptance.md`. The desktop reference dispatcher remains a deterministic contract endpoint, not a physical board. |
