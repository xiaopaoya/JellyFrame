...
| Canvas 2D | Planned/optional capability | `graphics.canvas2d` is a registered standard capability name for apps that need `<canvas>` / `CanvasRenderingContext2D`. The runtime does not implement Canvas yet, and built-in target presets currently mark it unsupported, so declaring it produces an explicit package/check warning. The planned implementation uses lazy backing storage: pixels are allocated only when a page has a canvas and JS calls `getContext("2d")`; apps that do not declare or use Canvas should not pay runtime drawing cost. |
