...
| Frame dirty diagnostics | Works | Win32 scripted capture reports the per-run dirty flag distribution (`tree`, `attributes`, `text`, `style`, `layout`, `paint`, `render_or_layout`). Use this to find whether an app is spending frames on layout-producing DOM changes or cheap paint-only updates. |
