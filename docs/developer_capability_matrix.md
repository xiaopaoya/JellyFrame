...
| `overflow` | Subset | `visible`, `hidden`, `clip`, `auto`, `scroll`; clipping creates layer boundaries. V0 native scroll containers support fixed-size vertical `auto`/`scroll` regions with host-provided scroll offsets, clipped paint and hit testing. The Win32 shell maps wheel/arrow default actions to the nearest scrollable container before falling back to page scroll. Inertia, visible scrollbars, horizontal scroll and optimized container-only dirty repaint are deferred. |
