...
| Element-specific reflected attributes | Subset | Common low-cost per-element IDL reflections are available where they are plain content attributes: `meta.name/content/httpEquiv/media`, `data.value`, `time.dateTime`, `img.alt`, anchor `text/download/ping/rel`, plus anchor `referrerPolicy` as string reflection without browser policy normalization. Navigation, resource loading, image decoding, ping delivery and full URL utilities remain absent. |
