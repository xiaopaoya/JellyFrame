...
| Network loading | Host-optional XHR V0 | Core still has no HTTP, WebSocket or remote resource loading, and remote HTML/CSS/script/image resources cannot enter the page loader. `NetworkFetchMock` provides a fixture/handle/completion contract; the selected script runtime exposes an async `XMLHttpRequest` GET subset only when the host binds a service admitted by `network.fetch` and its host profile. Network rejection/completion failures classify into stable diagnostics such as `capability-denied`, `invalid-url`, `resource-not-found`, `offline`, `response-budget-exceeded`, `response-handle-budget-exceeded`, `request-timeout` and `request-cancelled`. Real networking belongs to host services/workers, and JS callbacks run only after UI/main task completion pumping. |
| Storage | Host-optional localStorage V0 | No cookies, IndexedDB or filesystem API in core. `AppPrivateKvStorageMock` provides app-id-isolated async KV semantics and budget checks. The selected script runtime exposes a tiny `localStorage` subset only when the host binds a non-blocking `AppLocalStorageShadow` admitted by `storage.kv` and its host profile; otherwise it is absent. Storage failures classify into stable diagnostics such as `capability-denied`, `invalid-key`, `value-budget`, `quota-exceeded`, `not-found`, `handle-budget-exceeded`, `operation-timeout` and `operation-cancelled`. `AppStorageLifecyclePolicy` and `apply_app_storage_lifecycle(...)` define when hosts flush, drop, delete or retain pending/persistent app storage during suspend, exit, crash, uninstall, update and memory pressure, with stable system-shell diagnostics such as `storage-flush-ok`, `storage-flush-failed`, `storage-drop-pending`, `storage-delete-data` and `storage-retain-data`. |
...
| Font coverage check | Default preflight with controlled auto-import | `package`, `check`, `preview` and source-package `install` run `jellyframe_font_resource_check` by default and use `--font-subset auto` to write `*.used_chars.txt` beside the report, plus a `fontSubset` plan in JSON. `--font-coverage` reports missing codepoints before embedding. Once the App declares the missing `.jffont` path, family, profile, axes and license metadata in manifest `fonts[]`, passing `--font-source-bdf` generates the subset and imports it into the `.jfapp` through a package overlay without modifying the source manifest or App directory. Use `--font-resource-id` when more than one declared font is missing; an unavailable licensed source, ambiguous declaration or remaining glyph gap is diagnosed explicitly. `--font-output` remains available when a separate generated artifact is required. `--no-font-check` or `--font-subset off` skips the path explicitly. Package reports include `fontDiagnostics`, `fontSubset` and `importedFontResources`. |
...
| Bitmap font pack generation | Tool/runtime/fallback chain works | `jellyframe_font_pack_gen` subsets BDF bitmap fonts into C++ `BitmapFont` headers for embedded builds and can also emit `.jffont` V0/V1 binary supplements. V0 is compact 1bpp. V1 stores opt-in 2bpp/4bpp glyph coverage for font-level antialiasing through `--coverage-bits 2|4`; 1bpp fonts keep the compact path and pay no coverage cost. `BitmapFontResource` can parse `.jffont` bytes, and `AppFontSet` exposes a bitmap fallback chain. Generic/no-family text tries the system font profile first, then app supplements for missing glyphs. A CSS `font-family` whose first family matches a manifest `.jffont` family hash tries that app font first, then falls back to the system/default chain. Win32 `--use-app-fonts` validates this layout/paint path. Stable `.jfapp`/flash font payloads can be attached through the zero-copy view path. Package diagnostics match explicit CSS `font-family` declarations against manifest metadata and verify statically identifiable CSS pixel sizes against each `.jffont` native line height. App-font rendering supports only integer scaling from the native size (1x through 8x); other requested sizes select the nearest scale and change text metrics. Synthetic bold is used for `font-weight >= 600`; full browser font matching, stretch/style/features and vector shaping remain future work. |
...
| CSS custom properties | Subset | Direct `var(--token)` and `var(--token, fallback)` are resolved from inherited `:root`/ancestor/current/inline custom property declarations. Unresolved or over-budget expansions do not override earlier supported fallbacks; resolved intermediate values are capped at 16 KiB by default. Full dependency graph, case-sensitive custom names and complete invalid-at-computed-value-time semantics are absent. |
...
## JavaScript / Script Runtime Binding

JavaScript support exists only when built with `JELLYFRAME_BUILD_SCRIPTING=ON`.
The build selects one script backend through `JELLYFRAME_SCRIPT_ENGINE`; the
current implementation is `jerryscript` and uses a local JerryScript tree
configured through `JERRYSCRIPT_ROOT`. Apps do not select a backend, and this
does not add runtime conversion or dispatch between script engines.
...
| Script execution watchdog | Host/runtime optional | `ScriptRuntimeOptions::max_execution_check_count` and `HostBudgets::max_script_execution_checks` define the framework budget. The current backend can interrupt runaway evals and callbacks with `script execution budget exceeded` when its linked JerryScript library was built with `JERRY_VM_HALT=ON`. If that capability is absent, JellyFrame reports the watchdog as unsupported and does not fake preemption. The Win32 validation shell can require this path with `--require-script-watchdog` plus bounded check options for recovery smoke tests. |
...
| Device Runtime / JFDP | Contracts plus one lifecycle-accepted port | `DeviceInstallTransaction`, `DeviceCapabilitySnapshot`, versioned request/result payload codecs and `JFDP/1` framing provide bounded install/lifecycle control, capability data, stable status/progress results and message integrity checks. WS147 native USB Serial/JTAG has accepted physical wire and persistent Developer Image lifecycle evidence at `743a011` (30 normal cycles, four hard reboots and eight controlled fault points), covering staging, redundant registry publication and protected-launcher recovery. The current Developer Image implementation routes installed static bundles to native UI and installed `runtime.script: "classic"` bundles to a worker-local JerryScript/DOM runtime that publishes sealed value frames to UI. That route awaits physical panel/input/fatal-recovery evidence and remains A2 `partial`; the desktop reference dispatcher is not a physical-board substitute. |
