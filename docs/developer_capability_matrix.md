...
| Flex | Subset | Simplified row layout with basic grow/shrink/basis sizing, justification, alignment, gaps and optional wrapping; non-wrapping column layout with the same main/cross-axis sizing controls. No full Flexbox algorithm, reverse directions, order, baseline alignment or min-content sizing. |
