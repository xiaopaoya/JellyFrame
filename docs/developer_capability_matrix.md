...
| Images | Host-optional/manual wiring | Platform-neutral `ImageDecodeMock`, `Surface` handle lifetime and width/height/decoded-byte/pending budgets now exist. Render core supports `ImageHandleResolver`, image display commands and `ImagePainter`, so hosts can map `<img src>` to decoded surfaces; automatic resource loading, surface cache and async repaint are still pending. |
