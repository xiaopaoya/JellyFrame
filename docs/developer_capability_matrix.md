...
| Bitmap font pack generation | Tool/runtime/fallback chain works | `jellyframe_font_pack_gen` subsets BDF bitmap fonts into C++ `BitmapFont` headers for embedded builds and can also emit `.jffont` V0 binary supplements; `BitmapFontResource` can parse `.jffont` bytes, and `AppFontSet` now exposes a bitmap fallback chain where the system font profile is tried first and app `.jffont` supplements fill missing glyphs. Win32 `--use-app-fonts` can validate in-bundle fonts for layout/paint. Stable `.jfapp`/flash font payloads can be attached through the zero-copy view path. CSS family matching and multi-size packs are still future work. |
