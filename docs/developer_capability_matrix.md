...
| Canvas 2D | Optional V0.1 capability | `graphics.canvas2d` declares bounded `<canvas>` / `CanvasRenderingContext2D` use. Render core provides `Canvas2DRegistry` with lazy RGBA backing stores, per-surface/total-pixel budgets, path-point/state-stack budgets and image-display-list integration. JerryScript builds expose `canvas.getContext("2d")`, `fillStyle`, `strokeStyle`, `lineWidth`, `globalAlpha`, `save`, `restore`, `clearRect`, `fillRect`, `strokeRect`, `beginPath`, `moveTo`, `lineTo`, `arc`, `closePath`, `fill` and antialiased `stroke` when the host binds a registry. Backing pixels are allocated only after `getContext("2d")`; apps without Canvas pay no drawing cost. Target presets still need to opt in with `hostServices.canvas2d`; unsupported targets produce package/check warnings. Canvas text, imageData, drawImage, transforms, gradients and compositing modes beyond source-over are still deferred. |
...
| SVG | Deferred | No SVG renderer. Use DOM/CSS boxes, gradients, images or optional Canvas 2D V0.1 for custom charts. |
