...
| Canvas 2D | Optional V0.3 capability | `graphics.canvas2d` declares bounded `<canvas>` / `CanvasRenderingContext2D` use. Render core provides `Canvas2DRegistry` with lazy RGBA backing stores, per-surface/total-pixel budgets, path-point/state-stack budgets, gradient/stop budgets and image-display-list integration. JerryScript builds expose `canvas.getContext("2d")`, `fillStyle`, `strokeStyle`, `lineWidth`, `globalAlpha`, `font`, `save`, `restore`, `clearRect`, `fillRect`, `strokeRect`, `beginPath`, `moveTo`, `lineTo`, `arc`, `closePath`, `fill`, antialiased `stroke`, `measureText`, `fillText`, `createLinearGradient` and `CanvasGradient.addColorStop` when the host binds a registry. Linear gradients are sampled for rectangles, paths and strokes; Canvas text uses one sampled color because it shares the host text-command backend with DOM text. Canvas text uses the same host text backend as DOM text when provided and falls back to the tiny built-in bitmap path otherwise. Backing pixels are allocated only after `getContext("2d")`; apps without Canvas pay no drawing cost. Target presets still need to opt in with `hostServices.canvas2d`; unsupported targets produce package/check warnings. imageData, drawImage, transforms, radial gradients, pattern fills and compositing modes beyond source-over are still deferred. |
...
| Host data snapshots | Host/system contract V0 | `AppHostDataSnapshot` defines a fixed-size, allocation-free host summary shape for battery, weather, activity, location and sensor summaries. `AppHostDataAccessPolicy` strips everything by default, then keeps only explicitly allowed battery/weather/activity fields and existing manifest/profile-gated location/sensor fields. This is not a JavaScript API yet; it is the host/system-shell boundary for future standard or product-profile surfaces. |
...
| App teardown/recovery | Works V0 | `AppRuntimeHost::terminate_current(reason)` cancels current-app requests, discards completions, releases host handles and clears app font resources with stable reason names such as `user-kill`, `script-watchdog`, `budget-exceeded`, `load-failure` and `system-policy`. The Win32 shell exposes `--system-survival-smoke N` to validate repeated bad-app budget recovery, stale completion filtering and launcher event delivery. |
...
| `background` | Subset | Solid colors, `linear-gradient(<color>, <color>)`, `linear-gradient(to bottom/top/right/left, ...)`, a two-segment progress `conic-gradient(<color> 0% N%, <color> N% 100%)`, and a two-color center-circle `radial-gradient([circle\|circle at center,] <color> [0%], <color> [100%])`. Conic gradients are intended for watch rings, battery rings and activity arcs; radial gradients are intended for hydrogel highlights, center glows and inset depth. Complex stops/angles/focal points/ellipses/repeating/multiple backgrounds are diagnosed and ignored without clearing earlier fallbacks. Out-of-subset conic/radial syntax emits `style-conic-gradient-unsupported` / `style-radial-gradient-unsupported`, and oversized paint areas emit `layer-conic-gradient-area-budget` / `layer-radial-gradient-area-budget`. |
| `background-image` | Subset | Accepts the same gradient subset as `background`, but not solid colors; use `background-color` or `background` for solid fills. |
...
| `overflow` | Subset | `visible`, `hidden`, `clip`, `auto`, `scroll`; clipping creates layer boundaries. V0 native scroll containers support fixed-size vertical `auto`/`scroll` regions with host-provided scroll offsets, clipped paint and hit testing. The Win32 shell maps wheel/arrow/drag default actions to the nearest scrollable container before falling back to page scroll, repaints only the container viewport when the dirty area stays within budget, applies a bounded host-side inertia tail while frame presentation is allowed, and enables opt-in lightweight scroll indicators. A conservative strip-blit fast path is used only for safe rectangular, opaque, non-overlapped containers; rounded/translucent/overlapped containers fall back to dirty repaint. Horizontal scroll is deferred. |
...
| `box-shadow` | Subset | `none` clears the shadow; the first shadow becomes an approximate rounded translucent fill. Real blur and multiple shadows are not rasterized; combine lightweight shadows, borders and `radial-gradient()` highlights for soft hydrogel surfaces. Large approximate shadow areas emit `layer-box-shadow-area-budget`. |
...
| Validation / form submission | Subset | `required`, text/textarea `minlength`/`maxlength`, checked required checkbox/radio groups and required select values are checked on `form.checkValidity()`, `form.reportValidity()` and submit activation. Invalid controls receive non-bubbling `invalid`; no browser popup is painted. `form.requestSubmit([submitter])` dispatches cancellable `SubmitEvent`-shaped `submit` with `submitter` after valid data collection. Browser navigation, action/method POST, multipart/file upload, pattern/type/date validation, `form.submit()`, reset and full `ValidityState` remain absent. |
...
| `textContent` / `innerText` | Works / Subset | `textContent` getter/setter; unchanged text avoids dirty work. A sole existing text child is updated in place; replacing mixed children remains structural. `innerText` is exposed as a lightweight `textContent` alias on element wrappers; it does not run the browser layout-aware rendered-text algorithm. |
...
| Form properties / `FormData` | Subset | Common IDL properties on relevant controls: `value`, `defaultValue`, input `defaultChecked`, `type`, `name`, `placeholder`, `required`, `checked`, `selectedIndex`, `readOnly`, `maxLength`, `minLength`, `min`, `max`, `step`, textarea `rows`/`cols`/`wrap`/`textLength`, select `size`, option `label`/`defaultSelected`/`value`/`text`/`index`, optgroup `label`, and progress/meter numeric `value`/`min`/`max` plus meter `low`/`high`/`optimum` and progress `position`. Form wrappers provide `checkValidity()`, `reportValidity()` and `requestSubmit()`. `new FormData(form)` and `append`/`set`/`delete`/`get`/`getAll`/`has` cover string form entries. Full `ValidityState`, labels collections, selection APIs, picker UI, file entries, iteration, reset and browser form navigation are deferred. |
...
| App-local route fragment | Subset | `window.location.hash` / global `location.hash` store one route fragment for the current running app. Changing its normalized value dispatches a non-cancelable `hashchange` on `window`; `window.onhashchange` shares the normal listener budget. The fragment resets when a new document binds. URL loading, `Location.assign/replace/reload`, `history`, navigation, browsing contexts and cross-app routes are absent. |
| App-local route fragment | Subset | `window.location.hash` / global `location.hash` store one route fragment for the current running app. Changing its normalized value dispatches a non-cancelable `hashchange` on `window`; `window.onhashchange` shares the normal listener budget. The fragment resets when a new document binds. URL loading, `Location.assign/replace/reload`, `history`, navigation, browsing contexts and cross-app routes are absent. |
...
| SVG | Deferred | No SVG renderer. Use DOM/CSS boxes, gradients, images or optional Canvas 2D V0.3 for custom charts. |
