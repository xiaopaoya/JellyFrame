...
| `window` / `document` | Subset | Bound objects expose the methods below. `window`, `self` and `document.defaultView` point to the same JellyFrame window object. Package documents expose `origin === "null"`, `isSecureContext === false` and `crossOriginIsolated === false`; browser URL/origin/security-policy machinery is not modeled. |
