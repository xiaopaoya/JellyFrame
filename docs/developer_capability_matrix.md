...
| Device Runtime / JFDP | Reference implementation | `DeviceInstallTransaction`, `DeviceCapabilitySnapshot`, versioned request/result payload codecs and `JFDP/1` framing provide bounded install/lifecycle control, capability data, stable status/progress results and message integrity checks. The desktop reference dispatcher is a deterministic contract endpoint, not a physical board or transport. Typed app-list/log-record/recovery-detail payloads, discovery, flash/storage, display/touch and telemetry remain official-port responsibilities. |
