...
> 最后更新：2026-08-30；适用版本：0.6.0-dev；当前开发线：0.6.0
...
| 响应式 | 有界契约 | 跨目标布局前先读 [responsive_layout_contract_zh.md](responsive_layout_contract_zh.md)。使用显式 `auto` 或省略宽高表达 intrinsic sizing，使用有限像素/百分比尺寸、LTR `inline-size` / `block-size`、`max-width: 100%`、`box-sizing: border-box`、`gap`、`aspect-ratio` 和四种文档化宽高 media 条件。`overflow-y: auto` 仅用于固定高度纵向滚动容器子集。 |
