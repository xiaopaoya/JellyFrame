...
1. M8：把运行循环和 dirty update 契约文档化并补测试。
2. M9：做更细的 invalidation/subtree reuse。
3. M10：补文本 backend adapter 和字体工作流验证。
4. M11：整理 app packaging。
5. M12：继续内存/allocator 优化。
6. M13：按硬件需求决定是否做 tiled presentation。
