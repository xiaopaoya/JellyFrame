...
| Device Runtime / JFDP | contracts + 一条已通过 lifecycle gate 的 port | `DeviceInstallTransaction`、`DeviceCapabilitySnapshot`、带版本的 request/result payload codec 与 `JFDP/1` framing 提供平台无关的有界安装/lifecycle control、capability 数据、稳定 status/progress result 与消息完整性校验。WS147 native USB Serial/JTAG 已在 `743a011` 通过 physical wire 与 persistent Developer Image lifecycle 证据（30 轮正常流程、4 次硬重启、8 个受控 fault point），覆盖 staging、冗余 registry publication 与 protected-launcher recovery。已安装 App 的 DOM/panel 渲染、device-side input 与作者工具接入仍属于 Device OS/A2 工作。桌面 reference dispatcher 仍只是确定性 contract endpoint，不是物理板卡。 |
