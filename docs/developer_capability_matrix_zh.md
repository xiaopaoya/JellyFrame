...
| 网络 | 宿主可选 V0 mock | 核心无 HTTP、XHR、WebSocket 或远程资源加载，仍不允许远程 HTML/CSS/script/image 进入页面 loader。`NetworkFetchMock` 已提供 fixture/handle/completion 契约；`app_service_policies_for_app(...)` 只有在 manifest 与 host/profile 同时允许 `network.fetch` 时才启用该能力；计划中的 JS 暴露优先做异步 `XMLHttpRequest` 子集，不做自定义 API。 |
| 存储 | 宿主可选 V0 mock | 无 cookie、IndexedDB 或核心文件系统 API。`AppPrivateKvStorageMock` 已提供 app id 隔离的异步 KV 契约和预算检查，用于设置、小型 JSON 状态和 token；`storage.kv` 需要 manifest/profile 合成后才会启用；计划中的 JS 暴露仅在目标能提供非阻塞 app 私有内存 shadow 时采用极小 `localStorage` 子集。 |
| 系统状态事件 | 宿主可选 V0 queue | `AppSystemEventQueue` 允许宿主为当前 app instance 注入有界的时间/时区/网络/电量/屏幕/低功耗状态快照。旧实例事件会在帧边界丢弃；JS callback 绑定仍延后。 |
...
| 字体覆盖检查 | 默认工具预检 | `package`、`check`、`preview` 和源码包 `install` 默认运行 `jellyframe_font_resource_check`；`--font-coverage` 在嵌入前报告缺失 codepoints，`--no-font-check` 可显式跳过。 |
...
| Bitmap font pack 生成 | 工具/runtime/显式验收可用 | `jellyframe_font_pack_gen` 把 BDF bitmap 字体裁剪成嵌入式构建可用的 C++ `BitmapFont` header，也可输出 `.jffont` V0 二进制补充包；`BitmapFontResource` 可解析 `.jffont` bytes，`AppFontSet` 可随 app instance 持有它，Win32 壳 `--use-app-fonts` 可让包内字体参与 layout/paint。产品级自动 fallback 绘制仍在路线图中。 |
...
| XHR/fetch/storage | 延后 | 当前只有 C++ 队列/mock。未来用户语法必须是标准子集：先做异步 `XMLHttpRequest`，`fetch()` 等 Promise/microtask 有界后再考虑；`localStorage` 只在目标能保证非阻塞时暴露极小子集。 |
...
| 图片 | 延后/宿主可选 | 核心暂不解码图片。`img`/媒体节点只有可用盒和 fallback。目标板可声明可选 host image decode service；接入后仍必须受尺寸、decoded bytes 和并发预算限制。 |
...
| Host budgets | 子集 | `HostBudgets` 已接入 HTML/CSS parsing、render/layout/layer tree 上限、display-list 上限、dirty rect 数量、frame-loop input/timer 上限、JerryScript timer/listener 限制、detached DOM node 限制和软件 compositor 的主/offscreen pixel 上限。Render/layout/layer tree 已有 arena-backed 构建路径；完整 mutable DOM arena 仍是后续工作。 |
