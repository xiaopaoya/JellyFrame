...
| 网络 | 宿主可选 XHR V0 | 核心仍无 HTTP、WebSocket 或远程资源加载，不允许远程 HTML/CSS/script/image 进入页面 loader。`NetworkFetchMock` 已提供 fixture/handle/completion 契约；只有 `network.fetch` 与 host profile 同时准入且 host 完成绑定时，`JerryScriptRuntime` 才在 scripting 构建中暴露 async `XMLHttpRequest` GET 子集。网络 request/completion 失败会归类为稳定 diagnostics，例如 `capability-denied`、`invalid-url`、`resource-not-found`、`offline`、`response-budget-exceeded`、`response-handle-budget-exceeded`、`request-timeout` 和 `request-cancelled`。真实网络由宿主 service/worker 完成，JS callback 只在 UI/main task pump completion 后执行。 |
| 存储 | 宿主可选 localStorage V0 | 无 cookie、IndexedDB 或核心文件系统 API。`AppPrivateKvStorageMock` 已提供 app id 隔离的异步 KV 契约和预算检查；只有 `storage.kv` 与 host profile 同时准入且 host 绑定非阻塞 `AppLocalStorageShadow` 时，`JerryScriptRuntime` 才暴露极小 `localStorage` 子集，否则不暴露。存储失败会归类为稳定 diagnostics，例如 `capability-denied`、`invalid-key`、`value-budget`、`quota-exceeded`、`not-found`、`handle-budget-exceeded`、`operation-timeout` 和 `operation-cancelled`。`AppStorageLifecyclePolicy` 和 `apply_app_storage_lifecycle(...)` 定义宿主在 suspend、exit、crash、uninstall、update 和 memory pressure 时如何 flush、drop、delete 或 retain pending/persistent app storage，并输出 `storage-flush-ok`、`storage-flush-failed`、`storage-drop-pending`、`storage-delete-data` 和 `storage-retain-data` 等稳定 system-shell diagnostics。 |
