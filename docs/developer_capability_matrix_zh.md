...
| 字体 profile 与预算估算 | 工具限定 | `jellyframe_capability_check --font-budget WxH` 会输出粗略 bitmap font pack byte 估算，并根据扫描到的码点建议 `tiny`、`tiny-plus-symbols`、`app-subset-cn`、`cn-standard` 或 `global-product`。 |
