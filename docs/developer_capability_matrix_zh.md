...
| 系统状态事件 | 宿主可选 V0 queue | `AppSystemEventQueue` 允许宿主为当前 app instance 注入有界的时间/时区/网络/电量/屏幕/低功耗状态快照。旧实例事件会在帧边界丢弃；`try_push_current(...)` 可诊断 `empty-instance` / `queue-full`。JerryScript V0 已将宿主时间映射到标准 `Date.now()`，并映射 `navigator.onLine`、`window` 的 `online`/`offline` 事件子集、`document.hidden`、`document.visibilityState` 和 `visibilitychange`。`TimeChanged` 只刷新宿主时钟，不派发 Web 事件。 |
