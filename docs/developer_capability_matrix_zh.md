...
| 网络 | 宿主可选 V0 mock | 核心无 HTTP、XHR、WebSocket 或远程资源加载，仍不允许远程 HTML/CSS/script/image 进入页面 loader。`NetworkFetchMock` 已提供 fixture/handle/completion 契约；`app_service_policies_for_app(...)` 只有在 manifest 与 host/profile 同时允许 `network.fetch` 时才启用该能力；当前尚未绑定 JS `fetch`。 |
| 存储 | 宿主可选 V0 mock | 无 cookie、同步 localStorage、IndexedDB 或核心文件系统 API。`AppPrivateKvStorageMock` 已提供 app id 隔离的异步 KV 契约和预算检查，用于设置、小型 JSON 状态和 token；`storage.kv` 需要 manifest/profile 合成后才会启用；当前尚未绑定 JS API。 |
