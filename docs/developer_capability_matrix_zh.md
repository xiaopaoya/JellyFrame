...
| 嵌入式后端边界 | 移植层持有子集 | 平台无关引擎已经暴露 framebuffer 转换、dirty rect、scroll-blit 计划、frame sink 边界、input-controller hook、文本 callback 和 host-service pump。真实 display flush、触摸/按键驱动、panel DMA、睡眠唤醒，以及 storage/network/audio/sensor 设备仍由 port 持有。 |
| 外链 CSS 加载 | 壳/宿主子集 | 宿主可通过核心 callback helper 加载本地 `<link rel="stylesheet">`；仍不支持远程加载。单个 document 只接受宿主配置的 stylesheet 数量和 CSS 聚合字节预算（`max_document_stylesheets` 与 `max_resource_bytes`）；超限的后续完整 stylesheet 会以 `css-document-resource-limit` 跳过，绝不会截断后交给 parser。 |
| Classic script 收集 | 宿主子集 | 启用 scripting 时，classic inline 和 package-local external script 按文档顺序收集。单个 document 只接受宿主配置的 script 数量和 source 聚合字节预算（`max_document_scripts` 与 `max_resource_bytes`）；超限的后续完整 script 会以 `script-document-resource-limit` 跳过，绝不会截断。模块加载和远程 script 仍不支持。 |
| 网络 | 宿主可选 XHR V0 | 核心仍无 HTTP、WebSocket 或远程资源加载，不允许远程 HTML/CSS/script/image 进入页面 loader。`NetworkFetchMock` 已提供 fixture/handle/completion 契约；只有 `network.fetch` 与 host profile 同时准入且 host 完成绑定时，选定脚本运行时才在 scripting 构建中暴露 async `XMLHttpRequest` GET 子集。网络 request/completion 失败会归类为稳定 diagnostics，例如 `capability-denied`、`invalid-url`、`resource-not-found`、`offline`、`response-budget-exceeded`、`response-handle-budget-exceeded`、`request-timeout` 和 `request-cancelled`。真实网络由宿主 service/worker 完成，JS callback 只在 UI/main task pump completion 后执行。 |
| 存储 | 宿主可选 localStorage V0 | 无 cookie、IndexedDB 或核心文件系统 API。`AppPrivateKvStorageMock` 已提供 app id 隔离的异步 KV 契约和预算检查；只有 `storage.kv` 与 host profile 同时准入且 host 绑定非阻塞 `AppLocalStorageShadow` 时，选定脚本运行时才暴露极小 `localStorage` 子集，否则不暴露。存储失败会归类为稳定 diagnostics，例如 `capability-denied`、`invalid-key`、`value-budget`、`quota-exceeded`、`not-found`、`handle-budget-exceeded`、`operation-timeout` 和 `operation-cancelled`。`AppStorageLifecyclePolicy` 和 `apply_app_storage_lifecycle(...)` 定义宿主在 suspend、exit、crash、uninstall、update 和 memory pressure 时如何 flush、drop、delete 或 retain pending/persistent app storage，并输出 `storage-flush-ok`、`storage-flush-failed`、`storage-drop-pending`、`storage-delete-data` 和 `storage-retain-data` 等稳定 system-shell diagnostics。 |
| App 分发 | 壳/宿主 contract V0 | CLI 可生成 `.jfapp`，并安装到桌面 registry mock。bundle 携带 package validation 生成的规范化 manifest summary；registry 安装和原生 Win32 bundle loader 都会在解释 app policy 前拒绝字段缺失/类型错误、未规范化 entry path 或 capability 投影漂移的 summary。Registry entry 现在带有产品语义字段，例如 `status`、`enabled`、`updatedAtUtc`、可选 `rollback` 和可选 `failure` 记录。`status` 仅限 `installed`、`disabled` 和 `failed`；`rollback-ready` 由 rollback 元数据派生，见 `tools/schemas/jellyframe.installed_apps.registry.schema.json`。更新 app 时会保留上一版 bundle 作为回滚目标；安装更低 `versionCode` 默认会被拒绝，必须显式 `--allow-downgrade`。`jellyframe_cli.py install --candidate` 与 Win32 壳 `--install-candidate PATH` 接受宿主下载并验证后的本地 candidate JSON；提交前会检查声明的 SHA-256、`trusted` 签名状态和用户批准。`tools/app_registry.py state` 会输出面向 launcher 的派生 state report。`tools/app_registry.py rollback` 和 Win32 壳 `--rollback-app ID` 可验收常规回滚路径。`enable` / `disable` 会保留 bundle 与数据，只控制是否可启动；启用 failed app 会清除 failure 记录。示例 Win32 壳在清数据或删除 app 前要求独立的宿主持有确认。删除 app 会删除当前 bundle 和 rollback bundle；是否保留 app 私有数据由显式参数决定。这仍是桌面/system-shell contract，不是完整 app store、签名权威、支付流程或 JavaScript 包管理 API。真实产品仍需负责下载、签名校验、用户授权、固件安全 staging 和持久 registry 存储。 |
| 授权文件访问 | 宿主 broker 设计 V0 | 普通 app 没有通用文件系统访问能力，不能影响 runtime 文件、系统组件或其他 app 数据。系统组件、文件管理器或用户批准的 app 应声明 `file.read`、`file.write` 或 `file.manage`，并通过宿主持有的 file broker。核心已提供 `AuthorizedFilePolicy`、`AuthorizedFileRequest` 和校验 helper，用于 capability gate、用户授权、规范化逻辑路径、字节预算，以及 `user-approval-required`、`capability-denied`、`invalid-path`、`traversal-rejected`、`byte-budget-exceeded` 等稳定错误。Win32 壳提供 `--authorized-file-smoke DIR`，用于验收未授权写入、路径穿越拒绝、staged commit/rollback 和 manage 操作 gate。真实产品 I/O 与授权 UX 仍由宿主实现；目前不暴露 JavaScript 文件 API。App 仍不能获得裸 filesystem、flash partition 或 block-device handle。 |
| 视频帧预览 | 宿主可选实验合同 | `media.video.frame` 是面向产品私有 preview surface 的有界 host-owned 最新帧合同。V0 runtime 接受 MJPEG，以及仅在显式启用时的 H.264 baseline；每个 source 同时只有一个 in-flight request，分配有界 RGB565 frame handle，替换帧无法分配时保留旧帧，替换成功后丢弃旧帧。它不提供 HTML `<video>`、`HTMLMediaElement`、track、JavaScript media API、core 内 codec，也不会影响未声明该能力的 app。target 通过 package/check diagnostics 报告 `hostServices.videoFrame`。 |
| Canvas 2D | 可选 V0.4 capability | `graphics.canvas2d` 用于声明有界 `<canvas>` / `CanvasRenderingContext2D` 使用。render core 提供 `Canvas2DRegistry`，包含按需 RGBA backing store、单 surface/总像素预算、path 点数/状态栈预算、gradient/stop 预算和 image display-list 集成；JerryScript 构建在宿主绑定 registry 后暴露 `canvas.getContext("2d")`、`fillStyle`、`strokeStyle`、`lineWidth`、`globalAlpha`、`font`、`save`、`restore`、`clearRect`、`fillRect`、`strokeRect`、`beginPath`、`moveTo`、`lineTo`、`arc`、`closePath`、`fill`、抗锯齿 `stroke`、`measureText`、`fillText`、`createLinearGradient`、`createRadialGradient`、`CanvasGradient.addColorStop`、`translate`、`resetTransform`、`quadraticCurveTo` 和 `bezierCurveTo`。linear gradient 会用于矩形、path 和 stroke 的逐像素采样；Canvas 文本因为复用 DOM 文本相同的宿主 text-command backend，会退化为在文本起点采样出的单色。Canvas 文本在宿主提供时复用与 DOM 文本相同的 text backend，否则退回极小内置 bitmap 路径。像素只在 `getContext("2d")` 后分配；不用 Canvas 的 app 不承担绘制成本。target preset 仍需用 `hostServices.canvas2d` 显式开启；不支持的 target 会输出 package/check warning。canvas-to-canvas `drawImage()` 支持标准 3/5/9 参数形式、裁切后的最近邻缩放和现有 `globalAlpha`；它要求 source canvas 已分配 surface，且不会新建 surface。`createRadialGradient()` 只接受同心圆上的双 stop；`translate(x, y)` 会把有限偏移按像素取整，作用于绘制/path/text/image 目标坐标，在创建 gradient 时捕获当前平移，并由 `save()`/`restore()` 保存；`resetTransform()` 只清除这一有界平移。`quadraticCurveTo()` 会把一段二次曲线 tessellate 为最多 24 个保留 path 点，`bezierCurveTo()` 会把一段三次曲线 tessellate 为最多 32 个；两者超过现有点数预算时均拒绝。`<img>`、`ImageBitmap` 与视频 source、self-copy、imageData、焦点/非同心或多 stop radial gradient、scale、rotate、通用矩阵、pattern fill 和 source-over 以外的合成模式仍延后。 |
| 系统状态事件 | 宿主可选 V0 queue | `AppSystemEventQueue` 允许宿主 producer 为当前 app instance 注入有界的时间/时区/网络/电量/屏幕/低功耗状态快照。producer push、UI frame pump、discard 与查询都受同步保护；producer 只读取 host 的原子 instance-id snapshot，不借用 lifecycle 或 DOM 状态。旧实例事件会在帧边界丢弃；`try_push_current(...)` 可诊断 `empty-instance` / `queue-full`。JerryScript V0 已将宿主时间映射到标准 `Date.now()`，并映射 `navigator.onLine`、`window` 的 `online`/`offline` 事件子集、`document.hidden`、`document.visibilityState` 和 `visibilitychange`。`TimeChanged` 只刷新宿主时钟，不派发 Web 事件。 |
| 宿主数据快照 | 宿主可选 V0 | `AppHostDataSnapshot` 定义固定大小、无分配的宿主摘要形状，覆盖 battery、weather、activity、location 和 sensor summary。`AppHostDataAccessPolicy` 默认清掉所有字段。仅当 host 显式绑定快照，且 manifest 声明 `system.battery`、`system.weather` 或 `system.activity` 时，`navigator.jellyframe.getSnapshot()` 才同步返回已批准的 battery/weather/activity 值；不可用字段为 `null`。调用不会创建 service request、订阅、轮询循环、callback queue 或 native handle。该私有命名空间刻意不模拟 Battery Status、Geolocation 或 sensor 浏览器 API。 |
| 宿主计算任务 | 宿主可选合同 V0 | `compute.jobs` 可声明有界的具名宿主工作。`AppComputeJobRequest` 只携带 operation name、有界字节输入和 timeout；宿主在 UI owner 之外执行，并通过既有 request/completion/handle 边界返回 result handle。`AppComputeJobPolicy` 限制 input bytes、result bytes 和每 app jobs；app teardown 时结果会被清理。这刻意不是 `Worker`、`MessagePort`、任意 callback code 或 JS API。 |
| 传感器/定位 | 宿主可选语义服务 V0 | App 可在 manifest 声明 `sensor.accelerometer`、`sensor.gyroscope`、`sensor.heart-rate`、`sensor.ambient-light` 或 `location.position`。只有 host/profile 同时允许时，平台无关 `AppSensorSampleMock` / `AppLocationSnapshotMock` 语义服务才启用。结果通过有界 request/completion 和 host handle 回 UI task；App 不会得到 GPIO/I2C/SPI/BLE/GPS 等裸硬件句柄。宿主绑定 location service 后，选定脚本运行时暴露 `navigator.geolocation.getCurrentPosition(success, error)` 子集；sensor JS API 仍延后。失败会归类为 `capability-denied`、`sample-unavailable`、`record-budget-exceeded`、`handle-budget-exceeded`、`request-timeout` 等 diagnostics。真实硬件采样频率、后台策略和授权提示由宿主决定。 |
...
| App teardown/recovery | 可用 V0 | `AppRuntimeHost::terminate_current(reason)` 会取消当前 app request、丢弃 completion、释放 host handle，并清理 app font resource；稳定 reason 包括 `user-kill`、`script-watchdog`、`budget-exceeded`、`load-failure` 和 `system-policy`。script-service request 会把内部 runtime client token 贯穿到 request、completion 和返回 handle；`clear_app_services()` 只释放该 runtime 自己的资源，并会回收其迟到 completion，不会误触同一 app instance 中其他 consumer 的资源。Win32 system shell 会在 watchdog 或 budget 耗尽终止已安装 app 后，将它标记为 failed 再返回 launcher；用户可重新启用、回滚或移除。`--system-survival-smoke N` 用于验收坏 app 多轮 budget recovery、stale completion 过滤和 launcher 事件投递。 |
...
| 错误恢复 | 子集 | 有明确资源上限。tokenizer 在构造 DOM 前限制每 tag 的属性数量，以及 tag 名、属性名和属性值字节数；超限内容会被继续消费但不会继续扩容，并报告稳定 diagnostic。tree builder 继续报告 node/depth/attribute 预算压力；畸形输入不应导致死循环或崩溃。 |
...
| `@font-face` | 懒处理 | 平衡跳过；不实现 CSS 字体加载规则。App 字体必须在 manifest `fonts[]` 中声明；当宿主使用 app-font 文本后端时，`.jffont` 条目可通过下方文档化的 `font-family` 子集参与 runtime 选择。 |
| 字体覆盖检查 | 默认工具预检与受控自动导入 | `package`、`check`、`preview` 和源码包 `install` 默认运行 `jellyframe_font_resource_check`，并以 `--font-subset auto` 在 report 旁写出 `*.used_chars.txt`，同时在 JSON report 中写入 `fontSubset` 计划。`--font-coverage` 在嵌入前报告缺失 codepoints。App 先在 manifest `fonts[]` 声明缺失的 `.jffont` 路径、family、profile、axis 和授权元数据后，传入 `--font-source-bdf` 即可生成 subset，并通过 package overlay 自动放入 `.jfapp`；不会修改源 manifest 或 App 目录。存在多个缺失字体时用 `--font-resource-id` 选择；无授权源、声明有歧义或生成后仍缺字时会明确诊断。`--font-output` 仍可保留独立生成物。`--no-font-check` 或 `--font-subset off` 可显式跳过。Package report 会输出 `fontDiagnostics`、`fontSubset` 和 `importedFontResources`。 |
...
| Pseudo-elements | 子集 | `::before` 支持极小的 generated-content 路径，可用于文本/counter 列表标记和短 generated text。`::after` 支持同一文本/counter 绘制子集，适合单位、角标和状态点。完整 generated-content layout、marker styling 和 selection styling 延后。 |
...
| `background-image` | 子集 | 接受与 `background` 相同的一层/两层 gradient 子集，但不接受纯色。也支持一个包内绝对路径 `url("/assets/image.bmp")`：它复用宿主拥有的图片 surface cache，`background-color` 保留为 fallback。单张包内图片可使用 `background-size: cover`、`contain` 或 `100% 100%`，简单 `background-position` 子集以及 `background-repeat: no-repeat`；选择的 fit、position 与采样模式复用既有 image display command，不会给未声明背景图片的页面增加字段、decoder、cache 或绘制工作。远程/data URL、相对路径、query/fragment/traversal、多 URL 层、平铺和任意尺寸表达式仍不支持。package report 会以 `backgroundImageDiagnostics` 指出非法或缺失的本地资源。 |
...
| Flex | 子集 | 简化 row 支持基础 grow/shrink/basis sizing、justify、align、gap 和可选 wrapping；不换行 column 使用相同的主轴/交叉轴 sizing controls。direct flex child 支持有符号整数 `order`。无完整 Flexbox 算法、反向方向、baseline alignment 或 min-content sizing。 |
| Grid cards/forms | 子集 | 响应式 auto-fit/minmax 卡片 grid、gap、最小 auto rows、span、`repeat(N, 1fr)`、`repeat(N, minmax(0, 1fr))`、固定/`fr` `grid-template-rows` 与正整数 `grid-column`/`grid-row` start/end/span placement。无 named lines/areas、subgrid、dense packing。 |
...
| `dialog` | 有界 modal 子集 | scripting 构建提供 `open`、`returnValue`、`showModal()` 和 `close([returnValue])`。每个 document 同时最多一个 `showModal()` dialog。宿主可为 Escape/back 调用 `request_modal_cancel()`，它先派发可取消 `cancel`，再派发 `close`。Win32 会在 rebuild 后重新安装 modal input gate，把 focus/hit test 限制在 dialog 内，并在关闭后恢复之前的 focus。不提供 `show()`、`requestClose()`、嵌套 modal、light dismiss、浏览器 top layer、backdrop 或完整 inert algorithm。 |
...
| Modal input gate | 宿主安装子集 | `InputController::set_modal_root()` 把 hit test、focus traversal 和 activation 限制到一个可见 dialog subtree。它不保存全局 inert 状态；只有宿主安装 root 时才工作。 |
...
JavaScript 只在 `JELLYFRAME_BUILD_SCRIPTING=ON` 时可用。构建通过
`JELLYFRAME_SCRIPT_ENGINE` 选择唯一脚本后端；当前实现为 `jerryscript`，并通过
`JERRYSCRIPT_ROOT` 配置本地 JerryScript 源码树。App 不选择后端，也不会在不同
脚本引擎之间产生运行时转换或分派。
...
| 表单属性 / `FormData` | 子集 | 相关控件上的常用 IDL 属性：`value`、`defaultValue`、input `defaultChecked`、`type`、`name`、`placeholder`、`required`、`checked`、`selectedIndex`、`readOnly`、`maxLength`、`minLength`、`min`、`max`、`step`、`willValidate`、`validationMessage`、`validity`，textarea 的 `rows`/`cols`/`wrap`/`textLength`，select 的 `size`，option 的 `label`/`defaultSelected`/`value`/`text`/`index`，optgroup 的 `label`，以及 progress/meter 数值 `value`/`min`/`max`、meter 的 `low`/`high`/`optimum` 和 progress 的 `position`。控件提供 `checkValidity()`、`reportValidity()` 和 `setCustomValidity(message)`；form wrapper 额外提供 `requestSubmit()` 和可取消 `reset()`。`new FormData(form)` 以及 `append`/`set`/`delete`/`get`/`getAll`/`has`/`forEach` 覆盖字符串 form entry。每个 `FormData` 在核心收集和 JS mutation 时都受 runtime entry/总 name+value 字节预算约束，默认是 32 entries / 4096 bytes，超限抛出 `RangeError` 且不保留部分收集结果。`forEach` 按 entry 顺序传入 `(value, name, formData)`，在有界 entry 快照上运行；回调中新加的 entry 留待下一次调用。`ValidityState` 快照刻意只包含已实现的标志；labels collections、selection API、picker UI、file entry、iterator 方法和浏览器表单导航延后。 |
| Timer | 可用 | 宿主泵动 `setTimeout`、`clearTimeout`、`setInterval`、`clearInterval`；callback budget 由宿主控制。 |
| 脚本执行 watchdog | 宿主/runtime 可选 | `ScriptRuntimeOptions::max_execution_check_count` 与 `HostBudgets::max_script_execution_checks` 定义框架预算。当前后端在链接的 JerryScript 使用 `JERRY_VM_HALT=ON` 构建时可中断失控 eval/callback，并给出 `script execution budget exceeded`；若该能力缺失，JellyFrame 会报告 watchdog 不可用，不伪造抢占。Win32 验证壳可用 `--require-script-watchdog` 和有界 check 参数强制验收这条 recovery 路径。 |
...
| Render Core advanced-forms profile | 构建期可选 family | `forms.advanced` 裁剪本地 constraint validation、custom validity、有界 `FormData`、`SubmitEvent`、`requestSubmit()` 及可取消的 form reset/default action。关闭时基础控件仍可输入，但激活不再 submit/reset，脚本层也不暴露 `FormData`、`validity`、`checkValidity()`、`requestSubmit()` 与 `reset()`。公开 C++ API 仍由安全 no-op stub 保持可链接；App 不能在运行时启用此 family。 |
...
| Target service support | 工具限定 | Target preset 可用 `hostServices.networkFetch`、`storageKv`、`audioPlayback`、`imageDecode`、`imageCodecs`、语义传感器和定位字段描述目标可选服务。`serviceIntent.targetSupport` 会按 `supported`/`unsupported`/`unknown` 报告 app 请求的服务支持状态，`imageDiagnostics` 会报告 package-local 图片资源的 codec 目标支持状态。当 app 请求明确 unsupported 的服务时，package/check 输出 `service-target-unsupported` warning。它是开发期兼容性提示，不是权限授权。 |
| Host service workers | 平台无关 pump | `pump_app_host_service_worker(...)` 提供极薄的真实宿主服务 worker 边界：一次只处理一个 `HostServiceJobKind`，completion 保留 request 身份，UI completion queue 已满时不弹出 request。若 request 已归 worker 后队列才满，其 completion 会留在已有的有界 in-flight slot，直到 UI frame 可投递，因此不会静默丢失。`HostHandleTable` 操作受同步保护，且只提供 copy-out 查询；worker 不得保留表内指针。DOM/JS/layout/framebuffer 始终只归 UI task。它不创建线程，也不执行 I/O。 |
| 轻量视频/MJPEG/H.264 | 实验/宿主可选合同 | `media.video.frame` 为 MJPEG 以及显式启用的 H.264 baseline 提供有界最新帧路径。每个 source 只有一个 request；替换帧无法分配时保留旧帧，替换成功后丢弃过期帧。`<video>` 仍不支持；ESP32-S3 H.264 复测仍属实验，不进入默认 profile。 |
| SVG | 包期图标子集；运行时延后 | 没有 SVG renderer 或 SVG DOM。`package --rasterize-svg` 可在输入符合文档化受限词汇时，把一个静态 package-local 图标源转换为普通生成 BMP；非法或动态 SVG 会以源路径给出可修复失败。自定义图表可使用 DOM/CSS 盒子、图片或可选 Canvas 2D V0.4。 |
...
| 宿主合并重绘 | 子集 | Win32 壳会在 input/script callback 后只对 dirty DOM 重绘。Win32 壳的 viewport scroll 会复用已有完整内容 framebuffer，并在可行时通过移动行和只复制新露出行来更新可见 blit buffer。port 可选择调用 `coalesce_dirty_rects_into(...)`，以有界的额外绘制面积换取更少的通用 present 调用；成本策略由 port 自己提供，未使用的 app 不会为此付费。 |
...
| Dirty rectangle repaint | 子集 | `dirty_region` 会通过对比旧/新 layout box，或对 paint-only 变化复用同一份 layout，为直接文本、属性、表单控件绘制变化计算有界重绘区域。瞬态的核心绘制弹层（例如 `forms.advanced` 的 select 弹层）是树结构变化全屏回退的明确例外：宿主必须同时传入旧、新 `LayerNode` tree，核心会把旧、新 `LayerReasonTransientOverlay` bounds 与控件本体 bounds 合并，因此打开和关闭都无需全屏重绘。真正的树结构变化仍保守重绘 viewport。若估算 dirty area 过大，宿主也可以选择全帧重绘，避免局部 flush 反而更贵。软件 compositor 会在 clip replay 前丢掉重复或被完全包含的 dirty rectangles，避免同一区域被反复清屏/重绘。 |
| Animation invalidation | 子集 | `animation_invalidation` 可根据上一帧/当前帧的 animation style overrides，在当前 layout tree 上生成局部 dirty rectangles，覆盖 opacity/color paint-only 动画和 translate/scale/rotate transform 前后位置。目标已拥有 layer 的 opacity-only 帧可复用该 layer 的 display list；其余动画更新仍保守重建 layer tree。 |
...
| Frame dirty 诊断 | 可用 | Win32 脚本化 capture 会报告本轮 dirty flag 分布（`tree`、`attributes`、`text`、`style`、`layout`、`paint`、`overlay`、`render_or_layout`）、`text_stable`、`style_stable`、`overlay_dirty` 等 frame-update 原因，以及最终 `layer_tree layers=N display_commands=N` 计数。开发者可据此判断页面帧耗时主要来自 layout 型 DOM 变化、便宜的 paint-only 更新、瞬态弹层，还是 display-command-heavy 页面。 |
...
| Device Runtime / JFDP | contracts + 一条已通过 lifecycle gate 的 port | `DeviceInstallTransaction`、`DeviceCapabilitySnapshot`、带版本的 request/result payload codec 与 `JFDP/1` framing 提供平台无关的有界安装/lifecycle control、capability 数据、稳定 status/progress result 与消息完整性校验。WS147 native USB Serial/JTAG 已在 `743a011` 通过 physical wire 与 persistent Developer Image lifecycle 证据（30 轮正常流程、4 次硬重启、8 个受控 fault point），覆盖 staging、冗余 registry publication 与 protected-launcher recovery。当前 Developer Image 实现会将已安装 static bundle 路由至 native UI，将 `runtime.script: "classic"` bundle 路由至 worker-local JerryScript/DOM runtime，再以 sealed value frame 交给 UI。该路径尚待实机 panel/input/fatal-recovery 证据，A2 仍为 `partial`；桌面 reference dispatcher 不能替代物理板卡。 |
...
- 需要大型外部子系统，例如完整 image decode、font shaping、无界 canvas。
