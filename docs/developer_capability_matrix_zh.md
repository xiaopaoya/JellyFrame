...
| App teardown/recovery | 可用 V0 | `AppRuntimeHost::terminate_current(reason)` 会取消当前 app request、丢弃 completion、释放 host handle，并清理 app font resource；稳定 reason 包括 `user-kill`、`script-watchdog`、`budget-exceeded`、`load-failure` 和 `system-policy`。Win32 壳提供 `--system-survival-smoke N`，用于验收坏 app 多轮 budget recovery、stale completion 过滤和 launcher 事件投递。 |
