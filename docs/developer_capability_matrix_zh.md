...
| Canvas 2D | 可选 V0 capability | `graphics.canvas2d` 用于声明有界 `<canvas>` / `CanvasRenderingContext2D` 使用。render core 提供 `Canvas2DRegistry`，包含按需 RGBA backing store、单 surface/总像素预算和 image display-list 集成；JerryScript 构建在宿主绑定 registry 后暴露 `canvas.getContext("2d")`、`fillStyle`、`strokeStyle`、`lineWidth`、`clearRect`、`fillRect`、`strokeRect`、`beginPath`、`moveTo`、`lineTo` 和 `stroke`。像素只在 `getContext("2d")` 后分配；不用 Canvas 的 app 不承担绘制成本。target preset 仍需用 `hostServices.canvas2d` 显式开启；不支持的 target 会输出 package/check warning。暂不支持 arc/text/imageData/transform/save/restore/fill path。 |
...
| SVG | 延后 | 无 SVG renderer。自定义图表可使用 DOM/CSS 盒子、渐变、图片或可选 Canvas 2D V0。 |
