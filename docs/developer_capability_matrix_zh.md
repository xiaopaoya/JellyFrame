...
| 宿主数据快照 | 宿主/system contract V0 | `AppHostDataSnapshot` 定义固定大小、无分配的宿主摘要形状，覆盖 battery、weather、activity、location 和 sensor summary。`AppHostDataAccessPolicy` 默认清掉所有字段，只保留显式允许的 battery/weather/activity，以及已经通过 manifest/profile gate 的 location/sensor 字段。这还不是 JavaScript API；它是未来标准入口或产品 profile surface 之前的宿主/system-shell 边界。 |
