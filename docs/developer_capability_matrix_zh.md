...
| `background` | 子集 | 支持纯色，以及 `linear-gradient(<color>, <color>)`、`linear-gradient(to bottom, ...)`、`linear-gradient(to top, ...)` 这类低成本垂直渐变绘制命令。图片、复杂 stop 和角度会被忽略或诊断，不会覆盖之前 fallback。 |
