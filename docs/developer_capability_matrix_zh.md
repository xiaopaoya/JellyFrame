...
| Device Runtime / JFDP | contracts + 一条已通过 wire gate 的 port | `DeviceInstallTransaction`、`DeviceCapabilitySnapshot`、带版本的 request/result payload codec 与 `JFDP/1` framing 已提供平台无关的有界安装/lifecycle control、capability 数据、稳定 status/progress result 与消息完整性校验。WS147 native USB Serial/JTAG 已在一个文档化 image/configuration 上通过 JFDP/1 wire matrix；它不是 Developer Image lifecycle release。persistent registry/staging、launcher recovery、typed library/log record 与作者工具接入仍是 Device OS 工作，受 `device_image_lifecycle_port_acceptance_zh.md` 约束。桌面 reference dispatcher 仍只是确定性 contract endpoint，不是物理板卡。 |
