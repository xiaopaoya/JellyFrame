...
| Device Runtime / JFDP | 参考实现 | `DeviceInstallTransaction`、`DeviceCapabilitySnapshot`、带版本的 request/result payload codec 与 `JFDP/1` framing 已提供平台无关的有界安装/lifecycle control、capability 数据、稳定 status/progress result 与消息完整性校验。桌面 reference dispatcher 只是确定性 contract endpoint，不是物理板卡或 transport。typed app-list/log-record/recovery-detail payload、discovery、flash/storage、display/touch 与 telemetry 仍由官方 port image 负责。 |
