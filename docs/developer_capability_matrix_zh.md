...
| Device Runtime / JFDP | 参考实现 | `DeviceInstallTransaction`、`DeviceCapabilitySnapshot` 和 `JFDP/1` framing 已提供平台无关的有界安装事务、capability handshake 数据、稳定结果码与消息完整性校验；CLI 的 `device --transport reference` 只操作桌面 registry reference endpoint，明确不代表真实板卡已可连接。真实 discovery、flash/storage、display/touch、logs 和 telemetry 必须由官方 port image 实现。 |
