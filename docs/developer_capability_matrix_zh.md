...
| Canvas 2D | 计划中/可选 capability | `graphics.canvas2d` 是已登记的标准 capability 名称，用于声明 app 需要 `<canvas>` / `CanvasRenderingContext2D`。当前 runtime 尚未实现 Canvas API，内置 target preset 标记为 unsupported，因此声明该能力会在 package/check 中得到明确 warning。计划实现时采用按需 backing store：只有页面存在 canvas 且 JS 调用 `getContext("2d")` 后才分配像素内存；未声明/未使用 Canvas 的 app 不承担运行时绘制成本。 |
