...
| 字体覆盖检查 | 工具限定 | `jellyframe_capability_check --emit-used-chars` 收集源码中的非 ASCII 字符；`--font-coverage` 在嵌入前报告缺失 codepoints。 |
| Bitmap font pack 生成 | 工具限定 | `jellyframe_font_pack_gen` 把 BDF bitmap 字体裁剪成嵌入式构建可用的 C++ `BitmapFont` header。 |
...
| `flex-wrap` | 子集 | `wrap`/`wrap-reverse` 启用简单行换行。完整 flex sizing 仍不支持。 |
...
| 简单固定 grid 列 | 子集 | 支持 `grid-template-columns: <length> 1fr` 及相近的 2-4 列 length/`fr` 模板，适合描述列表和设置表单。 |
...
| Flex row | 子集 | 简化 row，支持基础 justify、align、gap 和可选 wrapping。无完整 flex sizing。 |
| Grid cards/forms | 子集 | 响应式 auto-fit/minmax 卡片 grid、gap、最小 auto rows、span 和简单固定 2-4 列模板。无显式 placement、named lines、subgrid、dense packing。 |
...
| 文本测量 | 子集 | core 暴露 `TextMeasureProvider`；fallback 很小但按 UTF-8 码点估算。Win32 壳使用 GDI 测量。 |
...
| 能力验证器 | 工具限定 | `jellyframe_capability_check` 在桌面扫描 HTML/CSS/JS，并报告受支持子集、降级特性和不支持 API。它也可以输出非 ASCII 使用字符并验证字体覆盖。 |
...
| Dirty rectangle repaint | 子集 | `dirty_region` 会通过对比旧/新 layout box，或对 paint-only 变化复用同一份 layout，为直接文本、属性、表单控件绘制变化计算有界重绘区域。树结构变化保守重绘 viewport。 |
...
| Host budgets | 子集 | `HostBudgets` 已接入 HTML/CSS parsing、render/layout/layer tree 上限、display-list 上限、dirty rect 数量和 JerryScript timer/listener 限制。Render/layout/layer tree 已有 arena-backed 构建路径；DOM arena 和离屏 framebuffer 预算仍是后续工作。 |
...
- 不要把必需 UI 放进 `@container`、`@supports`、不支持/复杂 media query 或 `:has()` 等不支持 selector 函数里。
- 脚本保持同步、小型；数据由宿主提供。
