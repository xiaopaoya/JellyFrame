...
| 包内背景图片定位 | 子集 | 一张 package-local `background-image: url("/assets/image.bmp")` 可使用 `background-size: cover`/`contain`/`100% 100%`、既有简单 `background-position` 子集和 `background-repeat: no-repeat`。选择的 fit、position 与采样模式编码进既有背景图片字，并由既有 image display command 携带，因此不会增加 `Style` 字段、decoder、cache 或未声明背景图页面的工作。平铺、多图层和任意尺寸表达式仍延后。 |
