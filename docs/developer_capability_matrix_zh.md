...
| `background` | 子集 | 支持纯色、`linear-gradient(<color>, <color>)`、`linear-gradient(to bottom/top/right/left, ...)`，以及双色进度 `conic-gradient(<color> 0% N%, <color> N% 100%)`。Conic gradient 面向表盘环、电量环和活动弧线；复杂 stop、角度、repeating 和多重背景会忽略或诊断，不会覆盖之前 fallback。 |
...
| Conic gradient | 子集 | 两段顺时针进度绘制命令，从 12 点方向开始。只在实际使用时栅格化；普通矩形和线性渐变保持原快路径。 |
