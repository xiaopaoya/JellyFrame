...
| 嵌入式后端边界 | 移植层持有子集 | 平台无关引擎已经暴露 framebuffer 转换、dirty rect、frame sink 边界、input-controller hook、文本 callback 和 host-service pump。真实 display flush、触摸/按键驱动、panel DMA、睡眠唤醒，以及 storage/network/audio/sensor 设备仍由 port 持有。 |
