...
| 字体覆盖检查 | 工具限定 | `wearweb_capability_check --emit-used-chars` 收集源码中的非 ASCII 字符；`--font-coverage` 在嵌入前报告缺失 codepoints。 |
| Bitmap font pack 生成 | 工具限定 | `wearweb_font_pack_gen` 把 BDF bitmap 字体裁剪成嵌入式构建可用的 C++ `BitmapFont` header。 |
...
| 能力验证器 | 工具限定 | `wearweb_capability_check` 在桌面扫描 HTML/CSS/JS，并报告受支持子集、降级特性和不支持 API。它也可以输出非 ASCII 使用字符并验证字体覆盖。 |
...
| Dirty rectangle repaint | 子集 | `dirty_region` 会通过对比旧/新 layout box，为直接文本、属性、表单控件变化计算有界重绘区域。树结构变化保守重绘 viewport。 |
...
dirty 并重绘一次；这次重绘仍会走简化管线 pass，但 framebuffer 阶段已经能对非结构性变化做有界 dirty rectangle 局部重绘。
