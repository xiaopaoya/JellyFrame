...
| Canvas 2D | 可选 V0.2 capability | `graphics.canvas2d` 用于声明有界 `<canvas>` / `CanvasRenderingContext2D` 使用。render core 提供 `Canvas2DRegistry`，包含按需 RGBA backing store、单 surface/总像素预算、path 点数/状态栈预算和 image display-list 集成；JerryScript 构建在宿主绑定 registry 后暴露 `canvas.getContext("2d")`、`fillStyle`、`strokeStyle`、`lineWidth`、`globalAlpha`、`font`、`save`、`restore`、`clearRect`、`fillRect`、`strokeRect`、`beginPath`、`moveTo`、`lineTo`、`arc`、`closePath`、`fill`、抗锯齿 `stroke`、`measureText` 和 `fillText`。Canvas 文本在宿主提供时复用与 DOM 文本相同的 text backend，否则退回极小内置 bitmap 路径。像素只在 `getContext("2d")` 后分配；不用 Canvas 的 app 不承担绘制成本。target preset 仍需用 `hostServices.canvas2d` 显式开启；不支持的 target 会输出 package/check warning。imageData、drawImage、transform、gradient 和 source-over 以外的合成模式仍延后。 |
...
| SVG | 延后 | 无 SVG renderer。自定义图表可使用 DOM/CSS 盒子、渐变、图片或可选 Canvas 2D V0.2。 |
