...
| App 分发 | 壳/宿主 contract V0 | CLI 可生成 `.jfapp`，并安装到桌面 registry mock。Registry entry 现在带有产品语义字段，例如 `status`、`enabled`、`updatedAtUtc`、可选 `rollback` 和可选 `failure` 记录。`status` 仅限 `installed`、`disabled` 和 `failed`；`rollback-ready` 由 rollback 元数据派生，见 `tools/schemas/jellyframe.installed_apps.registry.schema.json`。更新 app 时会保留上一版 bundle 作为回滚目标；安装更低 `versionCode` 默认会被拒绝，必须显式 `--allow-downgrade`。`tools/app_registry.py rollback` 和 Win32 壳 `--rollback-app ID` 可验收常规回滚路径。`enable` / `disable` 会保留 bundle 与数据，只控制是否可启动；启用 failed app 会清除 failure 记录。删除 app 会删除当前 bundle 和 rollback bundle；是否保留 app 私有数据由显式参数决定。这仍是桌面/system-shell contract，不是完整 app store、签名权威、支付流程或 JavaScript 包管理 API。真实产品仍需负责下载、签名校验、用户授权、固件安全 staging 和持久 registry 存储。 |
