...
| 元素专用反射属性 | 子集 | 对纯内容属性形式的常见低成本 IDL 反射提供支持：`meta.name/content/httpEquiv/media`、`data.value`、`time.dateTime`、`img.alt`、`label.htmlFor`、anchor 的 `text/download/ping/rel`，以及以字符串形式反射但不做浏览器 policy 规范化的 anchor `referrerPolicy`。导航、资源加载、图片解码、ping 发送、完整 label-control 关联和完整 URL 工具仍不实现。 |
