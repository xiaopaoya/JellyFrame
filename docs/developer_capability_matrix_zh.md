...
| 图片 | 宿主可选/未接绘制 | 已有平台无关 `ImageDecodeMock`、`Surface` handle 生命周期和尺寸/decoded bytes/pending 预算检查。`img`/媒体节点目前仍只有可用盒和 fallback；下一步把资源 loader、surface cache 和 display-list image command 接起来。 |
