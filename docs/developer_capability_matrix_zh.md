...
| 字体覆盖检查 | 工具限定 | `jellyframe_font_resource_check --emit-used-chars` 收集源码中的非 ASCII 字符；`--font-coverage` 在嵌入前报告缺失 codepoints。 |
| 字体 profile 与预算估算 | 工具限定 | `jellyframe_font_resource_check --font-budget WxH` 会输出粗略 bitmap font pack byte 估算，并根据扫描到的码点建议 `tiny`、`tiny-plus-symbols`、`app-subset-cn`、`cn-standard` 或 `global-product`。 |
| Bitmap font pack 生成 | 工具限定 | `jellyframe_font_pack_gen` 把 BDF bitmap 字体裁剪成嵌入式构建可用的 C++ `BitmapFont` header，并输出 glyph/byte 估算。 |
