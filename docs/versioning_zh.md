...
| 版本流 | 示例 | 契约 owner |
| --- | --- | --- |
| Render Core | `0.6.0`（已发布） | Core API/ABI、feature profile schema 和渲染行为 |
...
- `0.6.x`：设备路径和引擎边界收束阶段。首个带签名 Render Core `0.6.0` release 与
  Runtime lock 已关闭；本线负责 JFDP/Device Runtime 契约，以及满足 profile 与证据门槛的
