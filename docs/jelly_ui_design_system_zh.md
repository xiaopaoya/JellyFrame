...
- 克制使用：有界 `@keyframes` / `animation-*` from/to 动画，属性限于
  `opacity`、`transform: translate()/scale()/rotate()`、`background-color` 和 `color`。
  它适合少量常驻 loading/pulse 状态，不适合 layout motion。
- 可依赖：软件合成器对 composited layer 执行 `translate()/scale()/rotate()`，平移会取整，
  缩放和旋转使用受支持的 `transform-origin` 子集。
...
- 不依赖：`backdrop-filter`、CSS `filter`、`mix-blend-mode`、SVG 纹理、
  Canvas 绘制、layout 属性动画、skew、matrix、perspective、px 长度
  `transform-origin`。
...
- `samples/apps/packages/jelly_controls`：完整可安装 source package，展示原生文本、滑块、
  复选框和当前会话的保存反馈。
...
| Token | 时长 | 属性 | 用途 |
| --- | --- | --- | --- |
| `--jf-motion-press` | 140-180ms | `transform`, `background-color`, `opacity` | 按钮、按键、switch thumb |
| `--jf-motion-float-in` | 200-260ms | `opacity`, `transform` | dialog、toast、popover |
| `--jf-motion-settle` | 180-240ms | `transform` | 开关、滑块、卡片轻微复位 |
| `--jf-motion-pulse-core` | 900-1400ms | `opacity`, `background-color` | 少量 active/loading 状态 |
...
- skew、matrix、perspective 和 3D transform
