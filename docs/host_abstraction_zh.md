...
ESP32-S3 推荐路径是：JellyFrame software `FrameBuffer` -> `embedded_framebuffer` RGB565 转换 -> `flush(Rect)`/`packed_flush(Rect)` -> `esp_lcd_panel_draw_bitmap` 或等价屏幕驱动调用。输入从 board queue 进入 `InputController`；文本通过 `TextMeasureProvider` 和 `TextPainter` 注入。如果厂商 SDK 强绑定 LVGL，只包装最终 panel/input/text hooks，`src/core` 不应包含 LVGL 头文件。
...
1. 完善文本后端 adapter 和字体工作流验证。
2. 整理本地资源包与 app packaging。
3. 继续 allocator 工作，并用 M9 诊断判断 retained subtree reuse 是否值得承担所有权复杂度。
4. 在真实硬件压力要求时，再细化 offscreen/tile buffer 策略。
