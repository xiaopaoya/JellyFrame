...
2. 评估 DOM node 分配策略，以及 document arena 是否值得引入到 parser/tree-builder 复杂度中。
3. 为重复 class pattern 添加 style sharing 或 computed-style cache。
4. 基于已有 dirty flags 复用 render/layout/layer 子树，避免每次 mutation 后重建完整管线。
5. 添加 dirty layer invalidation 和 rectangle flush。
6. 添加 Win32/GDI 之外的 embedded bitmap/LVGL/vendor 文本后端示例。
