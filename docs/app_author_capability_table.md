...
| Local forms / `FormData` | `forms.advanced` profile subset | Requires the target Render Core profile to advertise `forms.advanced`. Use `form.checkValidity()`, `reportValidity()`, `requestSubmit([submitter])`, a cancellable `submit` event with `event.submitter`, and `new FormData(form)`. String entries support `append`, `set`, `delete`, `get`, `getAll`, `has` and `forEach`; the latter is a bounded snapshot, not a live iterator. Each object defaults to 32 entries / 4096 bytes and throws `RangeError` when the budget is exceeded. Send data only through an allowed host service from the event handler. |
