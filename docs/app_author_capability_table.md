...
| Canvas 2D | Host optional subset | `graphics.canvas2d`. Backing storage is allocated only after `getContext("2d")`. Includes canvas-to-canvas `drawImage()` in 3/5/9-argument forms with nearest-neighbor scaling, two-stop concentric `createRadialGradient()`, and pixel-aligned `translate(x, y)` retained by `save()`/`restore()`. `<img>`, `ImageBitmap`, video sources, focal/off-center or multi-stop radial gradients, scale, rotate, generic matrices and pixel APIs remain deferred. |
