...
| Canvas 2D | Host optional subset | `graphics.canvas2d`. Backing storage is allocated only after `getContext("2d")`. Includes bounded canvas-to-canvas `drawImage()` in 3/5/9-argument forms with nearest-neighbor scaling; `<img>`, `ImageBitmap` and video sources remain deferred. |
