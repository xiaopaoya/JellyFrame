...
| Local forms / `FormData` | Form V0 subset | Use `form.checkValidity()`, `reportValidity()`, `requestSubmit([submitter])`, a cancellable `submit` event with `event.submitter`, and `new FormData(form)`. String entries support `append`, `set`, `delete`, `get`, `getAll`, `has` and `forEach`; the latter is a bounded snapshot, not a live iterator. Send data only through an allowed host service from the event handler. |
