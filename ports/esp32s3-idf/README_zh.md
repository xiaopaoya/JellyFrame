...
板级驱动通过固定容量的 `BoardInputQueue` 传递触摸和其他输入。连续的
`PointerMove` 采样只保留相邻序列中的最新坐标，`PointerDown`、`PointerUp`、
滚轮、文本和焦点事件仍保持顺序。这样可以避免刷屏较慢时拖动事件积压，滑块
会跟随最新触点，而不会在手指抬起后继续重放旧坐标。队列满时会优先淘汰
最旧的可替换移动采样，再接收最新坐标或抬手事件，避免丢失结束拖动所需的
`PointerUp`。
