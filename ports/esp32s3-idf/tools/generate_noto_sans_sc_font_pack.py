...
def compact_coverage() -> list[str]:
    """Return the compact UI set used by sub-16px bitmap faces.

    Full GB2312 coverage is retained for the readable 16/20/24px faces. The
    small faces exist for dense, Latin-script system UI and must remain within
    the WS147 factory application partition.
    """
    chars = set(chr(codepoint) for codepoint in range(0x20, 0x7F))
    chars.update(COMMON_SYMBOLS)
    return sorted(chars, key=ord)
...
// Source font: Noto Sans SC. Compact faces use ASCII and UI symbols; 16px and
// larger faces use GB2312 level 1, GB2312 symbols, and ASCII.
...
"""
        ...
        for face in faces:
            chars = compact_chars if face.size in compact_sizes else full_chars
        ...
        out.write(f"const std::size_t kNotoSansScCoverageCount = {len(full_chars)};\n")
        ...
        coverage_name = "gb2312-level1-symbols-ascii-compact-small"
        if bits_per_pixel != 1:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--compact-ascii-sizes", default="")
...
def main() -> int:
    ...
    compact_sizes = {
        int(value) for value in args.compact_ascii_sizes.split(",") if value.strip()
    }
    if not compact_sizes.issubset(set(sizes)):
        raise SystemExit("--compact-ascii-sizes must be a subset of --sizes")
    weight_paths = [
    ...
    full_chars = default_coverage()
    small_chars = compact_coverage()
    ...
    print(
        "generated "
        f"{stats['faces']} faces, {stats['coverage']} full chars, "
