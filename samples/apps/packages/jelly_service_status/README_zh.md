...
用于验证可选数据/媒体服务边界的小型 runtime 示例。

它声明网络、存储和 MP3 播放能力，并通过 `backgroundServices` 表达 app 在
...
通过 `XMLHttpRequest` 请求 `/data/service-status.json`，并把最近一次服务状态写入
`localStorage` shadow，方便 Win32 壳同时验证系统状态投递、host completion 和小型非阻塞存储。
它不会假装已经存在公开的 JS 音频或传感器 API。
...
capture 汇总中应能看到非零 network host completion、脚本处理的 system event，以及
