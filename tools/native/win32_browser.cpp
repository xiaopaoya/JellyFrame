...
struct BrowserOptions {
    ...
    std::string enable_app_id;
    std::string disable_app_id;
...
public:
    ...
    int capture_frames() {
        ...
        std::cout << "JellyFrame Win32 browser frame capture\n"
                  ...
                  << '\n'
                  << "  frame_repaint dirty_rect=" << frame_repaint_statistics_.dirty_rect_frames
                  << " full_frame=" << frame_repaint_statistics_.full_frame_frames
                  << " full_first_paint="
                  << frame_repaint_full_frame_reason_count(frame_repaint_statistics_, FrameUpdateReason::FirstPaint)
                  << " full_tree_dirty="
                  << frame_repaint_full_frame_reason_count(frame_repaint_statistics_, FrameUpdateReason::TreeDirty)
                  << " full_framebuffer_mismatch="
                  << frame_repaint_full_frame_reason_count(frame_repaint_statistics_,
                                                           FrameUpdateReason::FramebufferSizeMismatch)
                  << " full_resolved_mismatch="
                  << frame_repaint_full_frame_reason_count(frame_repaint_statistics_,
                  ...
                  << " dirty_paint_dirty="
                  << frame_repaint_dirty_rect_reason_count(frame_repaint_statistics_,
                                                           FrameUpdateReason::PaintOnlyDirty)
                  << " dirty_text_stable="
                  << frame_repaint_dirty_rect_reason_count(frame_repaint_statistics_,
                                                           FrameUpdateReason::TextDirtyStableLayout)
                  << " dirty_style_stable="
                  << frame_repaint_dirty_rect_reason_count(frame_repaint_statistics_,
                                                           FrameUpdateReason::StyleDirtyStableLayout)
                  << " dirty_layout_previous="
                  << frame_repaint_dirty_rect_reason_count(frame_repaint_statistics_,
                                                           FrameUpdateReason::LayoutDirtyWithPreviousLayout)
                  << '\n'
...
private:
    ...
    bool process_shell_action_if_needed() {
        ...
        if (action == "launch") {
            ...
            try {
                const auto updated =
                    jellyframe_example::set_installed_app_enabled(options_.registry_store_path, app_id, enabled);
                configure_system_shell(std::string(enabled ? "Enabled " : "Disabled ") + updated.name + ".");
                rebuild();
                InvalidateRect(hwnd_, nullptr, FALSE);
            } catch (const std::exception& error) {
                configure_system_shell(std::string(enabled ? "Enable failed: " : "Disable failed: ") + error.what());
                rebuild();
                InvalidateRect(hwnd_, nullptr, FALSE);
            }
        } else {
    ...
    bool rebuild() {
        try {
            ...
            frame_repaint_statistics_ = FrameRepaintStatistics{};
    ...
    FrameRepaintPlan current_layout_repaint_plan(FrameUpdateReason reason, int content_height) const {
        FrameRepaintPlan plan;
        plan.dirty_rect_mode = FrameDirtyRectMode::CurrentLayout;
        plan.reason = reason;
        plan.target_width = viewport_width_;
        plan.target_height = content_height;
        plan.can_repaint_dirty_rects = true;
        return plan;
    }

    void record_frame_repaint(const FrameRepaintPlan& plan, bool used_dirty_rects) {
        last_frame_repaint_reason_ = plan.reason;
        record_frame_repaint_result(frame_repaint_statistics_, plan, used_dirty_rects);
    }
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--enable-app") {
            if (i + 1 >= argc) {
                std::cerr << "--enable-app requires an installed app id\n";
                return 1;
            }
            options.enable_app_id = argv[++i];
            continue;
        }
        if (arg == "--disable-app") {
            if (i + 1 >= argc) {
                std::cerr << "--disable-app requires an installed app id\n";
                return 1;
            }
            options.disable_app_id = argv[++i];
            continue;
        }
    ...
    if (!options.install_bundle_path.empty() || !options.install_candidate_path.empty() || !options.remove_app_id.empty() || !options.rollback_app_id.empty() ||
        ...
        if (options.registry_store_path.empty()) {
            std::cerr << "--registry-store is required for install/remove/rollback/enable/disable/delete-data/launch app manager commands\n";
            return 1;
        }
    }
    ...
    try {
        ...
        if (!options.enable_app_id.empty()) {
            const auto updated =
                jellyframe_example::set_installed_app_enabled(options.registry_store_path, options.enable_app_id, true);
            options.startup_status = "Enabled " + updated.name + ".";
            std::cout << "enabled " << updated.id << '\n';
        }
        if (!options.disable_app_id.empty()) {
            const auto updated =
                jellyframe_example::set_installed_app_enabled(options.registry_store_path, options.disable_app_id, false);
            options.startup_status = "Disabled " + updated.name + ".";
            std::cout << "disabled " << updated.id << '\n';
        }
