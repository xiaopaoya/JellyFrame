...
可穿戴设备形态上是否仍然可用。每个 profile 还会携带 pseudo-browser 首帧 `frameUpdate`
摘要，通常是 `first-paint` / `full-frame`，用来把预期内的首帧成本和后续 Win32 frame-loop
重绘回退数据区分开。pseudo-browser report 还会在原始 layout bounds 旁输出
