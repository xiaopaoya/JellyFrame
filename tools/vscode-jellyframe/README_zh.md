...
> 最后更新：2026-09-20；适用版本：0.6.0-dev；扩展版本：0.4.67；兼容基线：0.5.0
...
- 预览及外部调试启动显示进度通知；内嵌调试在首帧解码前显示加载动画与等待时间。
  超过 10 秒提示“启动较慢，仍在等待”，不误报失败或自动终止进程，内嵌调试仍可点击“停止”。
  首帧早于调试页面就绪时保留并补发，避免页面一直等待。
...
- `Render Trace` webview 可逐帧查看桌面壳 JSONL trace 的总耗时、阶段占比、dirty 区指标、
  ...
  缺失、截断或部分计时会在限制中明确显示。
- 内嵌调试提供显式的“性能跟踪 / 停止跟踪”控件。采样默认关闭；启用后原生壳最多在内存中
  保留最近 600 个已渲染帧或 4 MiB，停止时原子发布 JSONL 并自动打开现有查看器。
  整页 framebuffer 滚动显示为 `present-only / scroll-blit` 及实测 present span；
  内部滚动容器保留实测 layer、paint、present、dirty region 和 raster command 证据。
...
- “打开设备性能趋势”按 device case、profile、board、viewport 和 Core ABI 分组最近 20 次会话，
  显示 Frame/Paint/Present/DMA wait p95 的多会话折线与明细表，并将有界 JSON/HTML 写入
  `.jellyframe/build/performance/trends/`。缺失指标显示为空缺，不会补零或跨组连线。
...
作者环境状态同时显示 SDK 的 Render Core lock。App manifest 请求的 Runtime/Core line 与当前 SDK 不一致时，
验证、检查、打包、预览、调试、程控回放和设备部署会在启动命令前停止，并提供检查 SDK 更新或切换作者环境的操作。
SDK 安装不会覆盖已有目录；Windows 的短暂权限或文件占用会自动重试，仍失败时可选择重试、其他位置，或使用已验证的现有 SDK。
官方 App 作者 SDK 有意只携带预构建的 `desktop-release` 和 `desktop-scripting-release`，不携带
`CMakeCache.txt`；扩展会用 `sdk-manifest.json` 验证这两套运行时并直接使用。
...
容器、文本、按钮、包内图片、输入框、进度条、分隔线、留白、受限选择框、短列表、开关和小型导航栏；可以添加、调整顺序、嵌套、复制和修改属性，
也可以在 App 声明的视窗或常见设备尺寸下查看。保存结果是普通、可读的 HTML 和 CSS，
稳定元素 ID 可以继续供手写 JavaScript listener 使用。选项和列表内容通过有界的增删项目控件编辑，
不会暴露无上限的 JSON 输入框。画布下方还提供包含撤销、适应、缩放、结构和保存的简洁图标工具栏，
完整的文字操作仍保留在顶部栏。
生成的选择框仅属于单选子集；要使用 Render Core 绘制的选项浮层，目标必须声明文档中的
`forms.advanced` 能力，不提供浏览器原生多选或页面导航行为。
组件库还提供状态卡、设置行和底部导航三个透明模板组合。点击后会展开为普通可编辑节点，
模板只是起点，不会引入私有运行时组件。
新的 blank 和面向设备的物料默认使用黑色或近黑色表面，并保留明亮文字与强调色控件，
以减少小型圆屏边缘的视觉黑边。

右侧属性面板会显示包内脚本中可静态识别、与当前稳定 ID 对应的监听器，并可复制最小事件骨架。它不会修改 JavaScript，也不会猜测 App 的业务逻辑。
