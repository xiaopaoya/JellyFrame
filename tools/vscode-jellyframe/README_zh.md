...
“验证 App 包”默认是快速的包门禁：检查 manifest、入口、资源、引用和声明预算，适合频繁运行，
不会启动 Render Core，也不会测量布局、帧时间或真实设备性能。选择“程控回放”后，它会复用 `.jfcapture`
脚本驱动桌面壳，报告中记录脚本、运行日志、帧目录和蒙太奇结果。
“检查 App 渲染”会先做同样的结构验证，再运行目标 viewport 的 Render Core 预检、响应式布局和字体检查；
也可以附加程控回放，把静态管线诊断和多页面交互路径合并到一份报告。需要查看实际画面或交互时，请使用“预览”或桌面调试。
