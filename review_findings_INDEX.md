...
在 `272d66bd` 的 Windows Release microbench 上，对非 wrap Flex intrinsic layout
进行了三轮、每轮 200 次的 8/32/80 子项复测：

| 子项数 | 三轮平均耗时（us） | 每次布局文本测量 | layout boxes |
| ---: | --- | ---: | ---: |
| 8 | 12.005 / 12.975 / 13.255 | 56 | 28 |
| 32 | 47.160 / 45.180 / 50.160 | 224 | 100 |
| 80 | 109.640 / 71.440 / 87.450 | 180 | 244 |

80 项的测量调用数因宽度约束和布局分支不再按子项数线性增长，且三轮耗时波动明显。
当前没有足够稳定的语义前提证明可以引入全局 intrinsic cache 或叶子文本专用快路径；
本项标记为“已复测、暂缓实现”，只有后续固定设备 workload 显示它成为帧预算瓶颈时才
重新设计缓存失效规则。该结论不关闭其余 app service 载荷 API 设计和低优先级可读性条目。
