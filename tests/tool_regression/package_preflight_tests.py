...
class PackagePreflightTests(unittest.TestCase):
    ...
    def test_render_core_profile_preflight_rejects_missing_required_feature(self):
        manifest = package_app.validate_manifest({
            ...
            "runtime": {"minJellyFrame": "0.6.0", "minRenderCore": "0.6.0", "script": "none"},
    ...
    def test_manifest_rejects_a_historical_pre_release_runtime_line(self):
        with self.assertRaisesRegex(SystemExit, "must target the active pre-1.0 runtime line 0.6.0"):
            package_app.validate_manifest({
                ...
                "entry": "/index.html",
                "version": {"name": "0.1.0", "code": 1},
                "runtime": {"minJellyFrame": "0.5.0", "minRenderCore": "0.6.0", "script": "none"},
                "viewport": {"designWidth": 10, "designHeight": 10},
                "budgets": {"maxResourceBytes": 1024},
                "targets": {"test": {"viewport": {"width": 10, "height": 10}, "output": "jfapp"}},
            })
    ...
    def test_manifest_rejects_a_historical_pre_release_render_core_line(self):
        with self.assertRaisesRegex(SystemExit, "must target the active pre-1.0 Render Core line 0.6.0"):
            package_app.validate_manifest({
                "format": "jellyframe.app",
                "formatVersion": 0,
