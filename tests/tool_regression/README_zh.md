...
| 渲染性能报告与项目历史 | `render_performance_report_tests.py`、`device_performance_manifest_tests.py`、`device_performance_compare_tests.py`、`embedded_ui_workload_tests.py`、`vscode_performance_sessions_tests.js` |
...
runtime family，保持故意缺字 warning 稳定，并用 Win32 `--use-app-fonts` 捕获样例；它还会打包
`jelly_font_size_warning`，锁定插件安装提示所依赖的唯一 `font-size-unavailable` warning。
