...
- 可选 Canvas 2D V0.1 只在宿主绑定 canvas registry 后暴露：`canvas.getContext("2d")`、
  `fillStyle`、`strokeStyle`、`lineWidth`、`globalAlpha`、`save`、`restore`、
  `clearRect`、`fillRect`、`strokeRect`、`beginPath`、`moveTo`、`lineTo`、`arc`、
  `closePath`、`fill` 和 `stroke`。能力缺失或 backing-store 预算拒绝时，
...
- `fetch()`、模块、dynamic import、`sessionStorage`、IndexedDB、cookie、完整
  `HTMLAudioElement`、超出 `online`/`offline` 的完整 `Window`/`EventTarget` 语义、Canvas V0.1
