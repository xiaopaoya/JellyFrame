...
int LayoutEngine::layout_box(LayoutBox& box, int x, int y, int width, int height, std::size_t depth) const {
    ...
    // A button's anonymous inline content is laid out as ordinary inline
    // content, but the control itself has a vertically centered content box.
    // Keep the alignment at the layout boundary so every text backend receives
    // the same command rectangle (GDI, bitmap fonts, and injected painters).
    if (box.node != nullptr &&
        form_control_kind(*box.node) == FormControlKind::Button &&
        box.style.display != Display::Flex &&
        box.style.display != Display::Grid &&
        has_only_inline_children(box) &&
        content_height > children_height) {
        const int offset_y = (content_height - children_height) / 2;
        for (auto& child : box.children) {
            if (!is_out_of_flow_positioned(child->style)) {
                shift_box(*child, 0, offset_y);
            }
        }
    }
