...
`AppRuntimeHost::current_app_instance_id()` 的 pending record。这个规则覆盖 worker 已经在 teardown
前弹出的 request：它们已经不在 request queue 中，但仍不能在稍后分配旧 handle 或回调到后续 app
instance。storage 有独立的 flush/drop 生命周期策略，不应按普通 pending record 一刀切清理。
...
- 回归测试已覆盖 network、storage、image、audio、sensor、location 和 system event 的有界混合
  worker soak。这是 RTOS port 推荐参考形态：worker 只投递小 completion，UI 帧边界消费它们，
  stale 或已释放 handle 由服务生命周期 hook 清理。
- 生命周期 hook 应同时清理已释放结果 record 和“worker 已弹出但尚未完成”的 stale pending record。
  参考 mock 提供 `collect_stale_pending_fetches(...)`、
  `collect_stale_pending_decodes(...)`、`collect_stale_pending_commands(...)`、
  `collect_stale_pending_samples(...)` 和 `collect_stale_pending_snapshots(...)`。
...
- `AppImageCodecAdapter` 是产品级解码器边界。产品 host 可用它接 PNG、JPEG、WebP 或厂商 codec：
  输入 package-local 字节，输出 `AppImageCodecResult`。平台无关
  `app_image_codec_result_within_policy(...)` 会在结果变成可绘制 surface 前检查宽高、stride、
  pixel format、可选像素字节和 decoded-byte 预算。
...
- 如果 image worker 在 app teardown 前已经弹出 request，应调用
  `collect_stale_pending_decodes(...)`，避免旧 pending decode 继续占用 pending-decode 预算或稍后分配
  stale surface。
...
- 产品解码器必须运行在 UI/main task 之外，只通过小 completion 和宿主持有的 surface handle 回到 UI。
- 解码输出应优先匹配屏幕或 compositor 需要的格式。ESP32-S3 推荐 RGB565。
- 桌面端可以为方便走 RGBA 中转；小 MCU 上除非实测证明安全，否则应避免这种额外内存和转换成本。
...
- 真实 PNG/JPEG/WebP/厂商 decoder 仍属于 port 工作；JellyFrame 当前已经给出 adapter 形状和预算校验 helper。
...
- 它现在和 network、image、storage mock 一样提供 `complete_request(...)`：真实 host worker 可以先通过
  `pump_app_host_service_worker(...)` pop 出 `AudioCommand` request，再生成 completion；`complete_next(...)`
...
- 如果 audio worker 在 app teardown 时已经弹出 `AudioCommand`，应调用
  `collect_stale_pending_commands(...)`；旧 command 稍后返回时只能按 cancelled 处理。
...
- 服务 hook 还应清理 inactive app instance 的“已弹出但未完成”request，对应
  `collect_stale_pending_samples(...)` / `collect_stale_pending_snapshots(...)`。
...
- 如果 network worker 在 teardown 前已经弹出 fetch，应调用
  `collect_stale_pending_fetches(...)`；旧 worker completion 随后应按 cancelled 处理。
