...
`package`, `check` and `preview` run package validation first, then run a
...
app/package image. Pass `--font-budget`, `--font-coverage` or
`--emit-used-chars` when you also want the font resource audit before packaging
or previewing.
