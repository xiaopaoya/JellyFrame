...
- 目标 profile 可以按产品策略拒绝网络 app，或限制最大响应字节、并发请求和超时。
...
`package`、`check` 和 `preview` 默认先运行 package validation，再通过临时 render-core
...
app/package 图片。需要在打包或预览前做字体资源审计时，传入 `--font-budget`、
`--font-coverage` 或 `--emit-used-chars`。
...
桌面 installed-app registry mock 可以模拟系统 shell/app manager 的安装表：
...
复制到 staging，最后用原子 JSON 写入提交 `registry.json`。它不是最终图形化 app manager；
A3 会在 Win32 壳或独立桌面 shell 中使用这份 registry 显示、启动和删除 app。
...
package report 会记录 manifest 是否请求了网络能力，但目前还不会执行网络请求。
