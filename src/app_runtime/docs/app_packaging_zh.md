...
- `capabilities: ["media.audio.mp3"]` 声明可选的宿主持有 MP3 playback。当前 runtime
  已有 command/handle/completion 验证，但还没有面向用户的 JS 音频 API，也不内置 codec。
...
V0 已可由 runtime 解析并随 app instance 持有；`AppFontSet` 提供 bitmap fallback chain：
系统字体 profile 优先，app `.jffont` supplement 补缺字。Win32 壳可用 `--use-app-fonts`
显式切到这条验收路径。但 `@font-face` 和 `font-family` 仍不会自动切换文本后端，多 family/多字号的
产品策略仍是后续工作。当前稳定生产路径仍是：打包/检查阶段收集 used chars，离线从授权字体生成
bitmap glyph 数据，再把生成的 `BitmapFont` 编译进 port/firmware 并通过 `TextMeasureProvider`/
`TextPainter` 注入。
