...
6. 确认发现结果恰有当前 WS147 endpoint，`connected=true`，并依次选择“读取设备身份”和
   “列出已安装 App”。
...
- `list` 返回同一 endpoint 的 `registryGeneration` 与有界 App 列表；侧栏显示相同 generation、版本、
  state 与 rollback 摘要。空列表是有效的只读初始状态。
...
- JSON/JSONL shape、request ID、operation 或 selected endpoint mismatch：provider/host contract 问题。
...
provider ZIP SHA-256、registry generation 与 VS Code/插件版本。该冒烟验收不替代后续 install、live log 或
panel/input A2 验收。
