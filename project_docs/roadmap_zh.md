...
- Script Task value-frame v4 已合入 Runtime 主线。它保留有界定点 transform，并分离变换源空间与目标空间的 clip chain，覆盖变换层下的嵌套裁剪。WS147 独立验收已通过 300 次全帧 present、7 类畸形 frame 的精确拒绝与恢复，以及同源 P3 compatibility regression；dirty replay 明确为 `not-tested`，不能据此宣称 dirty 性能或 30 FPS。它不是 Developer Image 默认 profile，也尚不能单独关闭 A2 的 panel/input 证据。
...
value-frame v4 的 worker-to-UI 全帧 transform/source-clip/destination-clip 正确性与恢复移植验收已通过；归档为 `script_task_value_frame_v4_ws147_86c1135_20260827`。dirty replay 仍为独立的 `not-tested` 项，不能代替从 VS Code install/launch 或设备生命周期的作者流程证据，也不能关闭 panel/input 证据。
