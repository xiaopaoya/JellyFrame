...
- ESP32-S3 P4/P5/P6 smoke 支撑：bitmap font callback、有界开发板输入队列、焦点/文本/控件派发、
  dirty-rectangle RGB565 提交。当前内置 bring-up 字体只用于验证，不是生产中文字库。大体积生成字体包应由
  port/application 构建产生，并在单独审查后再决定是否进入主线。
...
- 文本 backend 已有默认路线。LVGL/vendor 集成刻意限制为可选的薄 text/panel/input hooks，
  不作为计划中的主渲染后端。
- app 打包格式和发布工作流还没有定型。
...
状态：核心可交付已完成。Dirty flag 清理和 dirty-region 遍历已改为显式工作栈，并按聚合 dirty 位剪枝，
...
M9 现在还暴露 `compute_dirty_region(...)`，提供明确的
clean/dirty-rect/full-frame mode 和 fallback reason，让宿主与测试能观察 dirty repaint 为什么无法保持局部。
它还提供保守的 dirty-area 成本 helper，使宿主能在局部 flush 已经覆盖大部分 framebuffer 时退回全帧重绘。
新增 `analyze_display_invalidation(...)` 诊断 dirty rectangles 命中了多少 layer/display command，
Win32 验证壳会显示 `cmds=命中/访问`，用于判断交互是否真的缩小 paint 工作。
完整 retained subtree/display-list reuse 仍是后续基于数据选择的工作，不再作为 M9 的阻塞项。

任务：

- 梳理 tree/style/layout/paint dirty flags 的传播边界。已完成第一版诊断和测试。
- 优先复用 render/layout/layer 子树中未变化部分。M9 已保守复用 paint-only 的 render/layout；
  retained subtree reuse 延后到有真实诊断数据后再做。
- 为 dirty layer/display-command invalidation 增加测试和诊断。已添加 display invalidation 诊断。
...
状态：主线可交付已完成。核心提供 `HostTextAdapter`，让 LVGL/vendor 字体引擎可以通过
统一 callback 包装接入 layout 测量和 software renderer 绘制。`jellyframe_capability_check`
现在会根据扫描到的码点输出 bitmap pack 粗略预算和字体 profile 建议（`tiny`、`tiny-plus-symbols`、
`app-subset-cn`、`cn-standard`、`global-product`）。`jellyframe_font_pack_gen` 会输出 emitted glyph 数、
row bytes、glyph table 估算和总估算，便于 port 文档记录。

任务：

- 保持 bitmap font backend 作为最低成本默认路线。
- 增加 LVGL/vendor text backend adapter 的平台无关示例或接口说明。第一版 `HostTextAdapter` 已完成。
- 完善字体覆盖、缺字、粗体、宽 glyph、标点和字号缩放测试。当前 bitmap backend 范围已补齐。
- 让 capability checker 更直接地报告字体包大小和缺字影响。当前工具范围已通过 profile 和预算输出完成。
...
`docs/app_packaging_zh.md`。第一版桌面 packer、伪浏览器 package 加载路径和示例 package
已经可用。
...
- 让 capability checker、font pack generator 和资源 bundle generator 能串成一条桌面构建链。
  packer 输出已包含结构化 resource/reference 数据，后续还需要统一 CLI wrapper。
...
1. M11：整理 app packaging。
2. M12：继续内存/allocator 优化，并把 M9 诊断证明值得做的 retained subtree reuse 纳入评估。
3. M13：按硬件需求决定是否做 tiled presentation。
