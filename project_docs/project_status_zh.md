...
状态：已开始并完成第一批主线能力。核心现在提供 `HostTextAdapter`，让 LVGL/vendor 字体引擎可以通过
统一 callback 包装接入 layout 测量和 software renderer 绘制。`jellyframe_capability_check`
新增 `--font-budget WxH`，可以按非 ASCII 子集估算 bitmap font pack 体积；`jellyframe_font_pack_gen`
会输出 emitted glyph 数、row bytes、glyph table 估算和总估算，便于 port 文档记录。

任务：

- 保持 bitmap font backend 作为最低成本默认路线。
- 增加 LVGL/vendor text backend adapter 的平台无关示例或接口说明。第一版 `HostTextAdapter` 已完成。
- 完善字体覆盖、缺字、粗体、宽 glyph、标点和字号缩放测试。已有 bitmap font 回归测试；仍可继续扩充标点/字号矩阵。
- 让 capability checker 更直接地报告字体包大小和缺字影响。第一版预算和覆盖汇总已完成。
...
1. M10：补文本 backend adapter 和字体工作流验证。
2. M11：整理 app packaging。
3. M12：继续内存/allocator 优化，并把 M9 诊断证明值得做的 retained subtree reuse 纳入评估。
4. M13：按硬件需求决定是否做 tiled presentation。
