...
2. **Render performance observability:** desktop trace production, bounded command/owner attribution, the practical frame-viewer workflow and Device Profile V0 are delivered. The WS147 `13264bcd` candidate now has a quantitative four-workload OFF/ON matrix with three repetitions per side and hash-verified operator photos supporting `visual-equivalent-only`; strict fixed-time equivalence, real touch-correlated drag/scroll windows and fair library baselines remain open. Aggregate device data must not be presented as per-element timing.
...
The 2026-09-14 matrix reports measurable frame/paint reductions for static, text and full repaint workloads, no change for the synthetic drag workload, and no error or present-failure regression. Twelve operator photos support no candidate-specific visual regression, but their missing side/phase classification and varying capture conditions keep the result at `visual-equivalent-only` rather than strict fixed-time equivalence. Shared green-text clipping remains in both drag fixtures; the synthetic drag fixture is not a gesture-latency result.
