...
The matrix passed on 2026-09-15 by human visual-equivalence. It reports measurable frame/paint reductions for static, text and full repaint workloads, no change for synthetic drag, zero error/present-failure regression, and no candidate-specific visual anomaly in twelve hash-verified photographs. The pixel-comparison waiver is scoped to this candidate and does not create an automated equivalence gate. Shared green-text clipping remains in both drag fixtures; the synthetic drag fixture is not a gesture-latency result.
