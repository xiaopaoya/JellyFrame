...
> 最后更新：2026-09-06；适用版本：0.6.0-dev
...
移植侧应针对该候选完成上述剩余 case；全部通过后再在一次受审查变更中同步更新 manifest/recovery 与 Provider 交付 metadata。在此之前，工具只能将候选视为 partial 的 `0.6.2` 镜像，历史镜像仍只能按 `0.6.1` identity 接受。详细执行顺序见英文交接文档。
