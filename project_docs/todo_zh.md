...
- [x] R1 基线复核已完成：独立工作树 `codex/r1-review` 的 Render Core-only Debug、Release、Sanitizer 构建均通过，CTest 均为 `9/9`。结论与剩余 RFC 见 `r1_render_core_audit_20260903_zh.md`；公共 dirty/clip 计数上限暂不以任意硬编码处理。
- [ ] 形成并评审 Render Core 统一预算契约 RFC：先盘点 Runtime、桌面壳、framebuffer 和 value-frame renderer 的实际预算，再决定是否进入 ABI 变更窗口。草案：`render_core_budget_contract_rfc_20260903_zh.md`。
- [x] 建立 `wrap_text_anywhere` 长度基线：窄栏与宽栏覆盖 32/128/512/2048 字符；当前基线增长接近线性。候选实现必须在同配置下重测并通过逐行等价校验，不能直接依据基线数字宣称优化收益。
